...
**MISSÕES (§228–§229), pronta:** o contador de missão **NÃO é o da maestria** — correção do dono, registrada (§228): a maestria conta TODA vitória incl. CPU (§215, local/cosmética); a **missão conta SÓ PvP** e **libera deus** (progressão), logo o **servidor é o dono**. **As 91 missões DERIVADAS dos dados** (`src/missoes_familias.js`): cada kit é classificado por **varredura do JSON** em UMA de **15 famílias-assinatura** (a mais distintiva vence; `dano` é o piso) + a **habilidade NOMEADA** que a encarna — não por tema (ex.: Apolo→cura/"Alvorada Radiante", tudo do kit). **A árvore com a raridade real** (`tools/gerar_missoes.js` → `data/missoes.json`): A 46 (camada 1: 23 · camada 2: 23) · S 30 · SS 15; A1→10 vit com um inicial+feito, A2→15 com um da camada 1+feito, S→15+feito·portão **Iniciado**, SS→20+**5 seguidas**·portão **Oráculo** — os portões lidos por CHAVE de `data/ranqueado.json` (§226), não literal. **As 8 cadeias à mão** (X→Y = Y destrava X, a passiva de X nomeia Y em 5 das 7): perseu→medusa · isis→osiris · mnevis→ra · raijin→fujin · inari→kitsune · change→houyi · hanuman→sunwukong · **odin exige 2+ Nórdicos**. **VALIDADA POR VARREDURA (§202), não por leitura:** DFS de cor + BFS dos 9 iniciais — **ZERO ciclo, os 91 alcançáveis** (o prereq default só aponta p/ rank menor; o gerador exclui do pool quem já depende por cadeia). **Alvos dos feitos CALIBRADOS na arena** (`tools/calibrar_missoes.js`), não chutados: IA-gulosa×gulosa mede, com o **mesmo leitor do servidor**, a métrica-assinatura por partida; alvo ≈ 5 partidas sólidas. **Elo Fase 4 (§189):** **25 dos 91 medem ~0** porque a IA gulosa não exercita os kits REATIVOS/passivos (reflexo, anti-revive, intercepta, roubo-de-orbe, campo) — os "slots mortos" recebem alvo-piso e ficam MARCADOS: F6 CONFIRMA o diagnóstico de F4 por medição independente. **O contador no servidor** (`server/missoes.js`+`server/contas.js`, aplicado em `salas.finalizarPartida` no fim do PvP): `vitoriasPvP`/`sequenciaPvP` (reset na derrota)/`paresPvP` (cadeias)/`feitos`/`liberados`, todos lidos do `st.log` autoritativo — `medir()` varre o log pelo **lado ativo + ator**, o que resolve o ESPELHO (mesma key nos dois times). **A GUARDA provada — o cliente não forja missão:** mensagem forjada (perfil.missoes) ignorada, partida inacabada não credita, desconexão=abandono=derrota (sequência reseta, sem vitória), idempotente (flag `registrado`), SÓ PvP (PvE credita zero). **Prova:** `tests/missoes.test.js` — 76 asserções; **38 suítes verdes**. **NADA DE TELA** (a pedido): só o contador, a árvore e a prova. Ferramentas: `npm run gerar:missoes`, `npm run calibrar:missoes`, `npm run test:missoes`.
