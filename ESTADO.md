...
**§284-ajuste:** (1) a **barra de maestria que chegava a 100% sem entregar** (Mestre exige 30 vit E o Milagre) — com vitórias cumpridas e só faltando o Milagre, a **barra SAI** e o requisito vira a manchete (`col2m--milagre`); medido 2 deuses nesse estado no perfil de teste; captura `e-milagre.png`; babá §6b. (2) **ABERTO (decisão do dono):** o kit é renderizado por DOIS caminhos — a sobreposição (`colKitLinhaHTML`, lê data/deuses) e a rota `'deus'` das Missões (`deusDetalheHTML`+chips, lê CKIT=kits.json+data/deuses). Não divergem hoje (o guarda da cadeia força), mas são 2 implementações. Opções com custo em DECISOES §284-ajuste — não mexido.
