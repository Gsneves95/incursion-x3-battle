...
**A VARREDURA de deus repetido (a pedido do dono) — deus em DESTAQUE de cada cartão:** campanha = **Odin** (nórdico: trono, corvos, runas) · provações = três medalhões, **Atena** (grega) ao centro + um deus-mar escamado + uma divindade solar · desafios = **Hércules** (grego) · invocação = só símbolos, nenhum deus · coleção = **Sun Wukong** (chinês) em destaque + a colagem cinza do acervo (proposital, é "seu panteão reunido") · loja = **Tanuki** (japonês) · batalha CPU = à esquerda um ancião barbado **com um corvo** tocando lira + à direita uma deusa alada egípcia · batalha PvP = **Fenrir** (nórdico) × **Kukulkán** (maia). **Nenhum deus-título se repete exatamente.** **A ÚNICA ambiguidade a reportar:** o ancião da metade esquerda de **batalha CPU** aparece com **corvo** — e corvo é o símbolo do Odin, que já é o destaque de **campanha**. O da CPU toca lira em vestes de poeta (não é a iconografia guerreira do Odin), então pode ser outro deus (um deus da poesia/bardo); mas para o jogador o corvo pode ler como "Odin de novo". **Como identifico deus por arte (incerto), deixo a decisão com o dono:** se o ancião da CPU não é o Odin, um ajuste pequeno (trocar/tirar o corvo) remove a leitura dupla; se preferir, digo qual era a intenção e conserto.
