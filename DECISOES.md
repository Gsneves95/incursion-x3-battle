...
### §282-b/c — o `arquetipo` que DERIVOU sem ninguém ver: espelhado em data/deuses, guardado, e virou legenda de tela.

O `arquetipo` ("Nuker de área", "Executor sombrio") existia nos 100 do **kits.json** mas NÃO em data/deuses, e o guarda de sincronia do §270/§271 (que confere faccao/elem/classe/funcao/nome/custo/recarga) **não o conferia** — a classe exata de defeito que aquele guarda existe para pegar, com um campo de fora. E custava conteúdo: num leitor-de-kit, o arquétipo é a legenda que orienta as 4 habilidades.

**(a) Auditoria cross-catalog (antes de tapar).** Descontados os renomes (elemento↔elem, tipo↔classe, custo↔cost, recarga↔cd, efeito↔desc), os campos presentes num catálogo e ausentes no outro: **só em kits** → `arquetipo` (texto de tela ← o buraco) e `num` (meta de planilha, ver abaixo); **só em deuses** (todos SINGLE-SOURCE do motor, kits é prosa e não pode divergir) → `inicial` (9), `provacao` (2), e por habilidade `slot`/`alvo`/`classe`/`fx` + modais (`modos`/`opcoes`/`alterna`/`classePorModo`/`cdSe`/`ignoraInalvejavel`/`umaVez`). **Veredito:** o único buraco da CLASSE do arquetipo — identidade duplicável que o guarda deixou passar — é o arquetipo mesmo, e é **texto de tela** (o motor NUNCA o lê — grep 0 em src/). Nenhum buraco de combate cross-catalog: o combate ou é single-source em deuses, ou (fx↔desc) já é guardado.

**Arsenal Celeste (nezha) — a desconfiança RESOLVIDA, registrada para não voltar.** A habilidade não tem `classe` plana; tem `classePorModo` + `modos` + `alterna` → a classe MUDA por modo. `classe` É lida pelo motor (a trava de Mágicas do Zeus, a redução do Oni), então ausência ALI seria buraco de COMBATE, não cosmético. Aqui é correto: o campo plano é substituído pelo por-modo. As 7 habilidades sem `fx` (exu/lugh/nezha/nuwa/tanuki×2/vishnu) são exatamente as modais/dinâmicas já declaradas fora do guarda plano no §271 (os ~5,4% multi/condicional). **Não refazer esta desconfiança.**

**O PORTÃO DO §263 (confirmado ANTES de aplicar).** `arquetipo` é TEXTO; o §263 diz que campo cosmético não invalida carimbo de Provação. A projeção de combate (`projecaoCombate`, src/provacao.js) é uma **deny-list**: campo novo ENTRA no hash por padrão ("erra para o lado de re-carimbar"). Então acrescentar arquetipo aos 100 deuses re-carimbaria as Provações **a menos que** ele seja declarado rótulo-de-tela. Declarei-o em `TELA_TOPO` (junto de `nome`/`curto`) — é comprovadamente de tela. **Medido:** `recarimbar_hash` dry-run → **mudariam 0 / 100 inalterados**; build sem aviso de "carimbo VELHO" → **as 91 Provações intactas, 0 re-carimbo**. Era a condição do dono: se ENTRASSE, parar; não entrou.

**(b) aplicado.** `arquetipo` inserido nos 100 data/deuses (cópia cirúrgica do kits.json, só +1 linha por arquivo, após `funcao`); `CHAVES_DEUS` (valida_kit.js) aceita o campo; `checar_cadeia.js` o compara como os outros metadados. **Babá provada:** muda um arquetipo num lado → `ERRO de cadeia ... [arquetipo] motor "…" ≠ prosa "…"`, build QUEBRA. Cadeia subiu de 1592 → **1692 conf** (+100).

**(c) na tela.** Subtítulo `col2p__arq` sob o nome, acima das tags (legenda que orienta o kit), itálico dourado-suave, uma linha. **Medido a 780 (Rajdhani real, rede bloqueada):** os **99 valores distintos — 0 cortam**; o mais apertado é "Protetor quase-invulnerável" (125px numa caixa de 207 → folga 82px). Nada a decidir.

**`num` (kits.json) — meta de planilha, DELIBERADAMENTE não espelhado.** É o ordinal 1–100 contíguo da planilha, **nunca lido em runtime** (grep 0). É o INVERSO do arquetipo: o arquetipo estava ausente por descuido (e foi tapado); o `num` fica de fora **por decisão** — não é identidade nem tela, é índice de origem. Registrado aqui para ninguém "consertar" espelhando-o depois achando que é deriva.
...
- **Conclusão:** dentro do que ficou congelado (hierarquia, agrupamento, retrato, arco a 47%, botão dentro do cartão), a cultura não entra no escuro. Como o número NÃO permitiu, **o contorno FICA** (o dono manda: "tire se o número permitir"). Não apliquei descida (o botão sairia — a regra do dono era PARAR e reportar).

**DECISÃO DO DONO: fica como está (opção a).** A "3ª via" (descer o bloco) falha pelo mesmo motivo das outras duas, só mais fundo. As capturas mostram as cinco legíveis; não se troca legibilidade comprovada por um número que mede outra coisa.

**§281-ajuste2 — cultura 9px → 11px (o que sobra de real): o contorno precisa de letra p/ trabalhar.** Contorno em 9px empasta (vira mancha antes de virar letra). Subi a cultura para **11px** e baixei o `letter-spacing` de `.28em` para **`.08em`** — **MESMA largura ocupada** (medido, pior caso "Japonesa" = 69px em 9/.28 e em 11/.08; nenhuma das cinco cresce; a Grega, a mais curta, 46=46). O botão NÃO se moveu (`entrarBot` 274 antes e depois; a cultura, sendo o topo, subiu ~3px sobre a arte, não empurrou nada para fora). Continua no nível SECUNDÁRIO (o nome tem 13 e manda).

**LIÇÃO (registrada porque vai voltar — a régua):** **contraste CRU mede texto PLANO sobre fundo.** Onde o texto tem CONTORNO, AURA ou SOMBRA, o número cru é um **PISO pessimista, não o veredito** — ele não enxerga o par texto+efeito, que é o que o olho lê. A verificação certa é a **captura sobre o pior caso real** (as cinco artes), não o número isolado. Aqui o 1,16 da cultura era isso: texto claro COM contorno sobre arte variável, medido como se fosse plano. Quase se "consertou" o que não estava quebrado. Regra para o futuro: número cru dispara a INVESTIGAÇÃO; a captura (ou uma medida do par texto+efeito, quando existir) dá o veredito.

**Arquivos deste ajuste:** `src/shell.html` (cultura 11px/.08em), `DECISOES.md`, `ESTADO.md`, `CLAUDE.md` (a régua). Suíte + build verdes.
...
**O que foi feito (verificado, verde):**
- **Tradução CHAVE→ASCII como função REUTILIZÁVEL** (`semAcento` = NFD + descarta diacríticos; `domArteArquivo(prefixo, cultura)`), não tabela de cinco linhas — as outras 5 culturas vêm depois (Africana/Brasileira sem acento, mas a Céltica pode ter). O cartão agora deriva o arquivo da arte e do emblema por essa função: `Egípcia`→`dominio-egipcia`, `Nórdica`→`dominio-nordica`. (Antes, o §276 montava o nome a partir da cultura acentuada e teria buscado `dominio-egípcia` — que nunca casaria com o arquivo ASCII.)
- **Guarda de projeto NOVA** `tests/assets.test.js`: varre `web/` inteiro e quebra se QUALQUER nome de arquivo tiver codepoint > 127 (babá: renomeie um asset com acento e cai). É a regra que o §279 quase descobriu do jeito difícil.
- **Guarda de mapeamento** `dominios.test.js` §12: `semAcento`/`domArteArquivo` corretas para Egípcia/Nórdica/Céltica, e os cinco nomes de arte são ASCII puro.
- **Pipeline PROVADO de ponta a ponta com fixtures descartáveis** (5 webp 1×1 ASCII, criados, build, conferidos e APAGADOS — não se commita arte falsa): a build registrou `DOMINIOS_ARTE={dominio-grega,dominio-egipcia,dominio-japonesa,dominio-nordica,dominio-chinesa}` e o runtime emitiu o `<img>` dos cinco (inclusive as duas acentuadas, resolvidas para ASCII), sem placeholder e sem 404.

**O que ficou PENDENTE (o anexo não chegou).** O ZIP das cinco artes (448×1030, 422 KB, `dominio-{grega,egipcia,japonesa,nordica,chinesa}.webp`) NÃO foi entregue a este contêiner — em duas tentativas, mesmo após reenvio (o contêiner foi recriado no intervalo). Sem os bytes reais NÃO dá para: copiar as cinco para `web/banners/dominios/`; reportar a tabela real cultura→arquivo→aceso; capturar a tela com a arte de verdade (o arco assentando sobre a cena); medir o peso do dist. A guarda do "lado cheio" (os cinco com arte PRESENTE, babá que quebra ao renomear) também depende dos arquivos e por isso ainda NÃO entrou — deixá-la agora deixaria a suíte vermelha sem culpa do código. **Tudo isso é drop-in trivial assim que os arquivos chegarem** (nomes ASCII já casam, acender é automático no próximo build).

**EMBLEMAS e FUNDO seguem ausentes de propósito:** emblemas no monograma latino (OL·AS·DU·TK·CE), tela correta sem o fundo (gradiente do jogo). **Não muda:** as 4 recusas, progresso no cartão, ciclo semanal, "Wukong", reticência Amaterasu/Tsukuyomi. **Arquivos:** `src/ui/home.js` (`semAcento`/`domArteArquivo` + arte/emblema por ASCII), `tests/assets.test.js` (novo), `tests/dominios.test.js` (§12), `package.json`, `CLAUDE.md`, `ESTADO.md`.
...
**MEDIÇÃO no piso 780 (Cinzel real, rede bloqueada), largura nova 134, caixa de deus ~44,7 (=134/3):** nome do Domínio (2 linhas; japonês em 3, sem corte), frase (2 linhas, a dos Céus cabe), linha de progresso e os três nomes de deus — **NADA passou a cortar que já não cortasse no §277**: seguem só Poseidon (1px), Amaterasu e Tsukuyomi (~6px, inteiros com reticência por decisão do dono no §277-ajustes); Wukong (40) cabe.

**§278-ajuste — a PÍLULA de progresso volta a UMA linha (aplicado).** A 134 a pílula quebrava em 2 linhas nas frases longas ("Semana · nível 38/40 ▲", "▸ Em corrida · nível 14/40") — e duas linhas numa pílula pequena viram mancha, logo no elemento que o jogador varre para comparar os cinco de relance. Encurtei o RÓTULO (não a fonte — reduzir a fonte pioraria a leitura de todos): "Semana ·" e "· nível" eram andaime; ficam o número e o SINAL de estado (ícone+cor+palavra). As quatro formas, medidas ≤118px (teto de uma linha a 780, com folga até o nível 40/40): **recorde da semana "Nível 40/40" (83), batido "Nível 40/40 ▲", corrida "▸ Em corrida 40/40" (101), e "Não iniciado" (71)** — este trocou "sem marca" (que soava a boletim) pelo termo da própria referência, que CONVIDA. A distinção à varredura segue firme: verde/▲ = recorde, âmbar/▸ = corrida, apagado = não-iniciado; "acessibilidade não se paga com pixel que sobra" — recusada a forma mínima "40/40 ▲" (que dependeria só da cor). **Guarda babá NOVA** (`moldura.test.js`, navegador real, rede bloqueada): nenhuma pílula quebra em 2 linhas nos QUATRO estados no piso 780 (semeados no pior caso 40/40; mede a altura real contra a de 1 linha) — foi o defeito que o §278 introduziu, e é do tipo que volta sozinho na próxima mudança de largura.
...
**BABÁS** `tests/dominios.test.js` §11: cartão 134 (proporção 0,435); véu é máscara SVG (arco), não linha reta; retrato arredondado (sem `clip-path`); rótulo de progresso curto ("Nível N/T"). `tests/moldura.test.js`: nenhuma pílula quebra em 2 linhas (4 estados, 780). Suíte + build verdes; **4 capturas** em `docs/capturas-277/` (a tela e um cartão em recorte, para bater de olho com a referência). **Arquivos:** `src/shell.html` (proporção, véu-arco, retrato), `src/ui/home.js` (rótulos de progresso curtos + "Não iniciado"), `tests/dominios.test.js`, `tests/moldura.test.js`, `ESTADO.md`.
...
- **1a (crescer a caixa) NÃO é viável.** No piso 780 o cartão tem 138px; a fileira dos três deuses já ocupa **142px na caixa de 44px** (encosta na borda). **TETO FÍSICO por nome = ~46px** (138÷3) — e vale para TODAS as culturas futuras. Caber "Sun Wukong" (60px) exigiria caixa 60 → fileira 190px → 52px para fora do cartão. Crescer arromba o cartão; descartado. Registrei o teto no `tools/gerar_dominios.js` (comentário do CFGS, onde a próxima cultura entra) e no ESTADO: **nome de deus > ~46px corta, e a saída é o `curtos` no dado, nunca a fonte nem a caixa.**
- **1b (encurtar no DADO — campo `curtos` da trinca, precedente `curto` do §262).** Medi os quinze (8px Cinzel 600, rede bloqueada). CORTAM quatro: Sun Wukong **60** · Amaterasu **52** · Tsukuyomi **52** · Poseidon **46**. **Decisão do dono, aplicada:** só **Sun Wukong → "Wukong" (40px)** — não é abreviação, é como ele é chamado; único caso limpo. **Amaterasu e Tsukuyomi FICAM INTEIROS**, com a reticência de ~6px: são nomes de uma palavra sem forma curta graciosa, e "Amateras" seria o nome ERRADO — a reticência SINALIZA que há mais texto (sinal > economia). **Poseidon FICA** a 2px do teto (some sem reticência perceptível). Fonte NÃO reduzida (pioraria 15 rótulos para salvar 4).
- **Implementação:** `curtos:{<key>:'curto'}` por escada (só a Chinesa tem, `{sunwukong:'Wukong'}`; as outras `{}`), do gerador ao dado; o CARTÃO usa `domNomeDeusCartao(lad,k)` (o resto — ex. frase do prêmio Reviver — segue com o nome inteiro).
- **§263 confirmado:** `curtos`/`curto` NÃO entra no carimbo de combate — `curto` está na deny-list `TELA_TOPO` de `projecaoCombate` (`src/provacao.js`), e o carimbo hasheia o catálogo de deuses/bestiário, nunca as escadas de `data/dominios`. Nenhuma Provação re-carimba.

**2. LINHA-GUIA — encurtada (aplicado).** Cortava 1px no piso 780 (544 vs 543). Tirei "· SEJA LENDÁRIO" (a parte que não descreve o modo — as outras três dizem o que ele é): agora **"ESCOLHA UM DOMÍNIO · ENFRENTE BATALHAS · ALCANCE O TOPO"**, medida **543/543 a 780 — cabe inteira** (e a 951/1200). Fonte intocada.

**3. PALETAS — aprovadas as duas.** Nórdica azul-gelo REGISTRADA no `src/shell.html` (`.dcard--nórdica`, junto das outras quatro, comentário "aprovada — não re-derivar"): `--tag:#BFE3F2 · --frame:rgba(150,205,230,.7) · --glow:rgba(90,165,205,.2) · --bodyA:rgba(10,20,32,.92) · --bodyB:rgba(6,12,20,.96) · --artFade:rgba(8,18,30,.9) · --veilTop:rgba(8,18,30,0) · --veilBottom:rgba(6,13,22,.94) · --embA:#1C3A4E · --embB:#08161E`. **Japonesa aprovada** no `emblemBg` **`#4A1030→#160610`** (vinho escuro — distingue do vermelho chinês sem brigar). Monogramas OL·AS·DU·TK·CE ficam (placeholder; somem com a arte).

**Arquivos:** `tools/gerar_dominios.js` (campo `curtos` + teto ~46px registrado), `data/dominios/*.json` (`curtos`; a Chinesa com `{sunwukong:"Wukong"}`), `src/ui/home.js` (linha-guia + `domNomeDeusCartao`), `src/shell.html` (comentário da Nórdica), `tests/dominios.test.js` (guardas do curto), `DECISOES.md`, `ESTADO.md`.

## §277 — A TELA DE ESCOLHA, 2ª leva: o mockup RICO do dono (chanfros, paleta por cultura, ouro por gradiente). Substitui o visual do §276; motor/ciclo/recordes do §275 seguem intactos.

**O que mudou sobre o §276.** A mesma tela (rota `dominios`), agora na linguagem cheia do mockup: cabeçalho com "Domínios" em Cinzel 900 dourado por `background-clip:text` (gradiente `178deg #FFFBEF→#F6E4AE→#E2C169→#A97C22→#F7E9B4`) + "?" de ajuda + linha-guia em Cinzel 600 `.24em`; seta de voltar como quadrado girado 45°. Cartões-pôster com **chanfro nos quatro cantos** (`clip-path`, 13px proporcionado ao cartão menor), **moldura+brilho por cultura** (`box-shadow` inset), dois véus sobre a arte, **emblema-anel** dourado meio-para-fora, **nome dourado em duas linhas** ("Domínio" / "do Olimpo"), frase itálica peso 300 com `text-wrap:balance`, **retratos com um canto chanfrado** e moldura dourada, **botão ENTRAR em bisel** (hexágono horizontal, fundo escuro + borda dourada). Rodapé de **dois painéis chanfrados**.

**NÃO ESCALEI O MOCKUP UNIFORMEMENTE (a advertência do dono, e ela procede).** O mockup é 1536×1024 (proporção 1,5 — quase quadrado); o palco é 951×428 (2,22 — o dobro de largo). Escalar por altura daria cartões de 120px e o nome do deus cairia a ~4,6px (metade do menor rótulo legível, §257). Mantive a LINGUAGEM e as PROPORÇÕES INTERNAS do cartão dentro das bandas do §276 (cab 48 · cartões 308 · rodapé 56, respiro 8), e o tipo escala por LEGIBILIDADE (piso do §enquadramento/§257/§261), não pelo fator geométrico: nome 15px, frase 9,5px, cultura 9px, deus 8px — os mesmos pisos legíveis do §276.

**As quatro mentiras do mockup — recusadas, agora na versão rica.** (1) o painel direito do rodapé dizia "DISPUTE O TOPO SEMANAL" com baú de "Recompensas" e "TOP 10", e o botão do topo dizia "RANKING SEMANAL" — **nada disso existe nem pode hoje (§274/§275)**; na mesma posição e peso: o botão do topo é **SEU HISTÓRICO**, o painel direito é **A MARCA A BATER** (recorde da semana passada + melhor de sempre), e o painel esquerdo é o seu melhor desempenho com um atalho **VER HISTÓRICO** (mesmo overlay). Sem baú, sem coroa, sem top-10, sem promessa de recompensa. (2) nenhum cartão "EM BREVE" — o ramo `locked` não se implementou (os cinco jogáveis desde o §274). (3) o progresso volta ao cartão, entre a frase e o botão. (4) o japonês é **TAKAMAGAHARA** (a arte dizia "Yomi"), trio Amaterasu/Susanoo/Tsukuyomi. **Frases novas do DADO** (as do §276 foram substituídas): Olimpo *"O raio, o tridente e a lança que pensa."* · Asgard *"O corvo, o martelo e a mentira."* · Duat *"A barca, a magia e o rei que volta."* · Takamagahara *"O sol, a tempestade e a lua."* · Céus *"A revolta, a lança e a mão que remendou o céu."*

**PALETA POR CULTURA (o que faz os cinco parecerem cinco lugares).** `--tag/--frame/--glow/--bodyA/--bodyB/--artFade/--veilTop/--veilBottom/--embA/--embB` numa classe `.dcard--<cultura>`. Valores do mockup para Grega (azul), Egípcia (âmbar), Chinesa (vermelho), Japonesa (magenta). **Nórdica o mockup não trouxe — DERIVEI em AZUL-GELO** (distinta da Grega: mais fria e mais clara): `--tag:#BFE3F2 · --frame:rgba(150,205,230,.7) · --glow:rgba(90,165,205,.2) · --bodyA:rgba(10,20,32,.92)→--bodyB:rgba(6,12,20,.96) · --artFade:rgba(8,18,30,.9) · --veil:rgba(8,18,30,0)→rgba(6,13,22,.94) · --emb:#1C3A4E→#08161E`. As cinco `--tag` são distintas (guarda). Da Japonesa o dono aproximou o final do `emblemBg`; ficou `#4A1030→#160610` — **confirme.**

**EMBLEMA-MONOGRAMA em LATINO (medição pendente do §276, resolvida).** Medi a cobertura dos glifos que eu havia posto (ΟΛ ☥ ⛩ 天 ᛟ) contra o `unicode-range` da Cinzel subsetada (§260 = latino + latino estendido): **os CINCO estão FORA do subset** (Ο/Λ são gregos U+039F/U+039B; ☥ U+2625; ⛩ U+26E9; 天 U+5929; ᛟ U+16DF) → caem na fonte do sistema e viram quadradinho no Android do dono. O dono pré-aprovou a forma ("duas maiúsculas em Cinzel"); troquei pelas INICIAIS do Domínio, todas cobertas: **OL (Olimpo) · AS (Asgard) · DU (Duat) · TK (Takamagahara) · CE (Céus)**. É o dono quem escolhe as letras — reportado, trivial de mudar. (São placeholders até `emblema-<c>.webp` existir.)

**MEDIÇÃO (Cinzel real, rede bloqueada, na largura MÍNIMA 780, cartão ~138) — dois cortes reportados, NÃO consertados (§257/§261: a escolha é do dono):**
- **Nome do deus corta em 4 dos 15.** Caixa de 44px, Cinzel 600 8px: **Sun Wukong** (precisa 60) · **Amaterasu** (51) · **Tsukuyomi** (51) · **Poseidon** (45, 1px) — saem com reticências ("Sun Wu…"). Cortam já a 951 também. As opções (fonte menor / caixa maior / apelido curto no dado / duas linhas) são do dono.
- **Linha-guia do cabeçalho corta 1px só no piso 780** (544 vs 543 → "…SEJA LENDÁRIO" começa a reticenciar); cabe inteira a 951/1200. Decorativa; a critério do dono (encurtar / reduzir fonte).
- **SEM corte, cabem em ≤2 linhas:** nome do Domínio (japonês em 3 linhas — "Takamagahara" não cabe na 2ª, sem corte), frase (a mais longa, a dos Céus, cabe), e a linha de progresso.

**ARTE por arquivo (portão §213), FUNDO opcional** — como no §276: build varre `web/banners/dominios/` → `DOMINIOS_ARTE`; runtime só emite `<img>` do que existe (nunca 404). Nada existe → tudo placeholder (a tela sem arte continua legível — guarda + captura provam). Fundo ausente → gradiente do jogo.

**BABÁS** `tests/dominios.test.js` §10 (as 13 do §276, seguem valendo) + §11 (§277: cada cartão tem `.dcard--<c>`; as cinco `--tag` distintas; todo monograma no latino coberto — sem tofu; rodapé de dois painéis com VER HISTÓRICO; "?" abre ajuda honesta; nome em dois `<span>`). Suíte + build verdes; **4 capturas** em `docs/capturas-277/`. **Arquivos:** `tools/gerar_dominios.js` + `data/dominios/*.json` (frases novas), `src/ui/home.js` (cartão/cabeçalho/rodapé/ajuda/monograma latino), `src/shell.html` (CSS §277), `tests/dominios.test.js` (§11), `ESTADO.md`.

## §276 — A TELA DE ESCOLHA dos Domínios: reskin em cartões-PÔSTER seguindo um mockup do dono. Troca de PELE + adaptação de CONTEÚDO — o motor e o ciclo semanal do §275 NÃO mudam.

**O que é.** A rota `dominios` (a seleção dos cinco) ganhou a linguagem visual que o dono aprovou: cinco cartões-pôster lado a lado, cada um com arte de tela cheia, um emblema circular meio-para-dentro/meio-para-fora no topo da moldura, a cultura em versalete, o nome do Domínio em Cinzel dourado grande, uma frase em itálico, os três retratos com nomes, e um botão ENTRAR. Acima, uma banda com título + um botão à direita; abaixo, uma barra-resumo. **A rota individual do Domínio, o ciclo semanal, os recordes por chave de semana, a corrida retomável e o invariante #3 NÃO mudam** — só a tela de escolha foi refeita.

**O mockup MENTE em quatro pontos — recusados de propósito (o motivo é o §274/§275, não capricho):**
1. **NÃO existe RANKING SEMANAL / TOP 10 / recompensa / baú.** Placar COMPARATIVO entre jogadores não existe e não pode existir hoje: nada sobrevive a um deploy no Render grátis e o dono decidiu não contratar store durável (§274/§275/§237). No MESMO lugar e peso visual: o botão do canto superior-direito virou **SEU HISTÓRICO** (abre os recordes semanais por Domínio que o perfil já guarda, dizendo em voz alta que é local e sem comparação com outros jogadores); a barra de baixo virou **A MARCA A BATER** (o Domínio de melhor desempenho, o recorde da semana passada dele, e o melhor de sempre — o jogador contra o próprio recorde, sem promessa de prêmio).
2. **NÃO há cartão "EM BREVE"/bloqueado.** Os cinco Domínios estão construídos e jogáveis desde o §274. Nenhum cadeado.
3. **O PROGRESSO volta ao cartão** (o §275 já o tinha): recorde desta semana, recorde anterior a bater, e a marca ▲ de batido — entre a frase e o botão.
4. **O Domínio japonês é TAKAMAGAHARA**, não "Yomi" (a imagem errou). A trinca é Amaterasu/Susanoo/Tsukuyomi. **Regra:** vale o DADO, nunca a imagem.

**GEOMETRIA (números do dono, no palco de 428).** cab 48 · cartões 308 · rodapé 56, com respiro 8 **só entre as bandas** (48+8+308+8+56 = 428, sem padding vertical externo — a minha 1ª tentativa pôs 8px acima+abaixo e comeu 16, deixando os cartões em 290; corrigido). Cinco cartões, margem 20 dos lados, 12 entre eles; emblema circular ~44 centrado no topo da moldura, meio-para-fora. **A largura é FLUIDA (design 780..1200), então os cartões FLEXIONAM** para cinco colunas: 172px é o valor a 951 (`max-width`), ~138px no mínimo 780, ~201px a 1200. Confirmado no dist, viewport real, fontes reais.

**MEDIÇÃO DE TEXTO (Cinzel real, rede bloqueada, na largura MÍNIMA — o caso apertado que o dono pediu para medir).** "Domínio de Takamagahara" (o nome mais longo) numa caixa de ~120px (cartão a 780): **quebra em 2 linhas e NÃO corta** (`scrollWidth == clientWidth` em todas as larguras — nenhuma palavra isolada, "Takamagahara" incluída, transborda a caixa). Todos os cinco nomes cabem em ≤2 linhas de 780 a 1200. **Não foi preciso decidir nada** (nenhum corte para o dono arbitrar).

**FRASES no DADO, não no código.** Um campo `frase` por cultura no arquivo da escada (`data/dominios/*.json`) + no gerador (`gerar_dominios.js` CFGS), lido pelo cartão. As cinco (propostas — o dono lê e pode vetar): Olimpo *"Glória, força e a vontade dos deuses."* · Asgard *"Batalha, destino e o crepúsculo dos deuses."* · Duat *"Vida, morte e o eterno equilíbrio."* · Takamagahara *"O sol, a tempestade e a lua velam o alto céu."* · Céus *"Disciplina, revolta e o poder sem limites."*

**ARTE POR ARQUIVO (o portão do §213/bestiário, reaplicado).** A build varre `web/banners/dominios/` e injeta `DOMINIOS_ARTE`; o runtime só emite `<img>` para o arquivo que existe — ausência = placeholder do §213, **nunca um `<img>` 404**. Nada de arte existe ainda, então a tela inteira roda em placeholder (o dono gera as imagens depois e cada cartão acende sozinho). Arquivos esperados: `dominio-{grega,egípcia,japonesa,nordica,chinesa}.webp`, `emblema-{...}.webp`, `dominios-fundo.webp`. **O fundo é OPCIONAL por desenho:** sem ele a tela usa o gradiente radial do jogo (o mesmo `.stage__bg`) e não fica feia. Emblema sem arte cai num monograma por cultura (símbolos: ΟΛ Olimpo · ☥ Egito · ⛩ Japão · 天 China · ᛟ Nórdica — símbolo, não a arte).

**BABÁS** (`tests/dominios.test.js` §10, 13 asserções): os cinco pôsteres aparecem e cada um abre o seu Domínio; NENHUMA banda promete recompensa/placar comparativo (sem RANKING/TOP 10/recompensa/baú); o botão do canto é SEU HISTÓRICO e ele diz "sem comparação com outros jogadores"; a barra é A MARCA A BATER; nenhum "EM BREVE"; japonês = TAKAMAGAHARA com a trinca certa; sem arte, cada cartão usa `.dcard__ph` e ZERO `<img>` de arte (sem 404); sem fundo, ZERO `<img>` de fundo; o progresso da semana volta ao cartão. Suíte + build verdes; **4 capturas** em `docs/capturas-276/` (progressos diferentes nos cinco + corrida em andamento; recorte do cartão em corrida; a tela sem nenhuma arte / estado limpo; o overlay SEU HISTÓRICO), em viewport real, fontes reais, rede bloqueada.

**Arquivos:** `tools/build.js` (varre `web/banners/dominios/` → `DOMINIOS_ARTE`), `tools/gerar_dominios.js` (campo `frase`), `data/dominios/*.json` (campo `frase`), `src/ui/home.js` (a tela-pôster: `renderDominios`/`dominioPosterHTML`/`dominioMarcaBaterHTML`/`abrirHistoricoDominios`/`temArteDom`/`embMonoDom`), `src/shell.html` (CSS `.dsel`/`.dcard`/`.dsel__marca`/`.dhist`), `tests/dominios.test.js` (§10), `ESTADO.md`.

## §275-bis — TRÊS REGISTROS do ciclo semanal (documentação; dois são análise reportada, não construção).

**1. RECORDE LOCAL NÃO É PROVA — nunca vira entrada de placar COMPARATIVO.** O relógio é do aparelho; quem o adianta abre semanas novas à vontade e acumula recordes locais. Hoje é inofensivo (não há comparação entre jogadores — quem burla só engana a si). Mas no dia em que o placar comparativo existir, **o histórico local NÃO pode ser subido para o servidor como pontuação** — ele é do jogador para o jogador, incalibrado e forjável pelo relógio. **Regra, junto da dependência de armazenamento durável (§275/§274/§237):** o placar do servidor, quando existir, **começa do zero e é o SERVIDOR quem conta a profundidade** (uma corrida ranqueada roda sob o relógio e a autoridade do servidor, como o ranqueado já faz com os pontos — §226: "o cliente NUNCA soma ponto"). O `perfil.dominios.porDominio[c].semanas`/`melhorSempre` são conveniência pessoal, jamais fonte de placar.

**2. HORIZONTE DE OITO SEMANAS — é PRAZO, não defeito.** Com N=8 e `semanas[idx mod N]`, na semana 9 o jogador rejoga a escada da semana 1 (chave de recorde nova, terreno conhecido): **dois meses de horizonte**. Por volta da semana 8 alguém precisa gerar mais (anotado no ESTADO como prazo). **Custo de gerar 52 semanas com o mesmo método:** a parte cara é o POOL de trios, medido UMA vez por cultura (não escala com as semanas); o marginal por semana medido foi **~8 s/cultura** (Grega 2 semanas 196 s → 8 semanas 244 s = +8 s/semana). Extrapolando das medições da Fatia 3 (8 sem × 5 = 930 s; pools ≈ 610 s no total, marginal ≈ 40 s/semana somando as 5): **52 semanas ≈ 610 + 52×40 ≈ 2 700 s ≈ ~45 min** (estimativa; ~40–60 min). Ou seja, **a esteira é ANUAL/trimestral, não mensal** — gerar um ano inteiro cabe numa rodada de fundo; não há por que rodar toda semana à mão.

**3. ESCADAS EMBUTIDAS × SERVIDAS COMO ARQUIVO (reporte, não construí).** Medido: 40 escadas inflaram o pacote de 2,02→2,17 MB (+~150 KB); na mesma proporção um ano das cinco (260 escadas) seria ~1 MB dentro do HTML. O projeto já moveu dado grande e frio para ARQUIVO duas vezes (as 401 artes de habilidade e os retratos do bestiário, §254) exatamente por isso, e escada é dado grande e frio, lido uma semana por vez. **Porém — o pacote NÃO cresce com o tempo se o horizonte for uma JANELA fixa:** como os recordes são keyed por semana REAL (`AAAA-Www`) e uma semana passada guarda só um NÚMERO (não uma escada rejogável), **não é preciso manter escadas velhas no pacote** — regerar com N≈8 mantém o bundle plano (~2,17 MB) para sempre, com conteúdo fresco (o `mod N` só reusa o SLOT, e a chave de recorde é real, sem colisão). O ~1 MB/ano só aparece se alguém DELIBERADAMENTE embutir um ano inteiro simultâneo. **O que custaria servir como arquivo, se um N grande simultâneo for desejado um dia:** uma escada por (cultura, semana) (~4 KB) carregada sob demanda como as artes — MAS com uma pegadinha que as artes não têm: `<img src>` funciona em `file://` (as artes), porém `fetch()` de JSON é bloqueado por CORS em `file://`, então uma escada BUSCADA quebraria o "abre sem servidor por `file://`" (o preview de dev e o modo de asset embutido do APK) a menos que a semana CORRENTE continue embutida como fallback. Ou seja, é mais do que mover artes: exige um caminho de carga assíncrono + estado "carregando semana…" + o fallback offline. **Recomendação: NÃO agora.** A janela fixa (N≈8, regerar periodicamente) já limita o bundle; servir como arquivo só compensa se o dono quiser muitas semanas simultâneas jogáveis — aí o desenho mais barato e seguro é "semana corrente embutida (fallback offline) + semanas extras como arquivo preguiçoso quando servido". Revisitar quando o horizonte crescer, não antes.

## §275 — DOMÍNIOS, Fatia 3: placar PESSOAL + ciclo semanal (tudo no perfil local; sem servidor, conta ou recompensa comparativa).

**A DECISÃO DE FUNDO.** A medição da Fatia 2 (§274) provou que NADA sobrevive a um deploy no Render gratuito. O dono decidiu não contratar armazenamento durável agora — então o placar é **PESSOAL**: o jogador compete com o próprio recorde da semana passada. Isso vive inteiro no perfil local (`localStorage`), que não passa pelo servidor e por isso não se perde num deploy.

**1) O CICLO SEMANAL, e como a semana é determinada SEM servidor (o ponto crítico — virada errada apaga progresso).**
- A semana é a **ISO-8601 do relógio do aparelho** (o mesmo mecanismo das Provações semanais). A CHAVE é `"AAAA-Www"`; o ÍNDICE absoluto (`ano*53 + semana`) incrementa +1 a cada semana ISO (inclusive na virada de ano) e escolhe a escada da semana: `semanas[indice mod N]`. Determinístico, offline, sem servidor.
- **Por que relógio errado / viagem de fuso NÃO apaga progresso:** os recordes NÃO são um par "atual/anterior" que uma "virada" sobrescreve (isso é frágil a relógio que oscila entre duas semanas — a marca viraria 0 num flap). São um **MAPA por chave** no perfil (`porDominio[cultura].semanas = { "AAAA-Www": profundidade }`), gravado sempre por **MAX**, e o **melhor de sempre** (`melhorSempre`) só cresce. Não existe evento destrutivo de "virada": virar a semana é só o mapa passar a LER outra chave. Relógio adiantado → abre uma chave nova (as antigas ficam). Relógio atrasado → lê/grava a chave velha por MAX (nada some). O mapa é limitado às ~12 semanas mais recentes, descartando a MAIS VELHA (nunca a atual/anterior). Pior caso de um relógio errado: jogar o mapa de outra semana — nunca perder recorde. **Testado com data adiantada E atrasada** (`perfil.test.js §275`).
- **"Recorde anterior" (a marca a bater)** = `semanas[chave da semana anterior]` (calculada por `hoje − 7 dias`). É o que dá sentido ao ciclo: com escada fixa, bater o próprio recorde é decorar o mapa; com escada nova a cada semana, é terreno novo.
- **Corrida EM ANDAMENTO na virada — não se perde, justificativa:** a corrida CARREGA a sua própria semana (`run.semana` + `run.semanaIdx`) e **termina na SUA escada**, mesmo que a semana vire no meio. A virada é INVISÍVEL para ela; ao terminar, a profundidade entra na chave da semana DELA e no melhor de sempre. Perder a corrida no meio seria o pior defeito — então a virada nunca toca a corrida em curso, só escolhe a escada de uma corrida NOVA.

**2) O QUE A TELA MOSTRA.** No cartão de cada Domínio (seleção): **recorde desta semana**, **recorde anterior** (a marca a bater), o selo **▲** quando já foi batido, e o **melhor de sempre** (marca que não zera — o peso a longo prazo, guardado separado do semanal). Quando a corrida passa a marca anterior, um **INSTANTE** comemorável (§264: progressão sem instante de pagamento é laço sem fecho): sobreposição dourada "RECORDE ANTERIOR SUPERADO", reusando a linguagem do banner de ranque, disparada UMA vez na corrida (o único momento comemorável do modo).

**3) AS ESCADAS DE VÁRIAS SEMANAS.** `tools/gerar_dominios.js --todas --semanas=N` gera N escadas por cultura, **mesmo método e mesma régua** (a fase 1); só a SELEÇÃO varia por semana (seed por cultura+semana), o **trio de cada Domínio é o mesmo em todas as semanas — rotação de trio NÃO entra nesta fatia**. Otimização da esteira: o POOL de trios (o caro) é medido UMA vez por cultura e reusado nas N semanas. **Gerei N=8 semanas × 5 culturas em 930s (~15,5 min)** (Grega 244s, Nórdica 155s, Egípcia 116s, Japonesa 271s, Chinesa 143s) — o custo cresce ~linear com as semanas depois do pool. **Cada escada de cada semana é monotônica** na dificuldade medida (a build valida, `domValidarLadder` loopa as semanas; `dominios.test.js §5` confere as 5×8). Publicado como um arquivo por cultura com `semanas: [{niveis, pisoIAGuloso}]` (JSON sem indent — 5×8×40 níveis; o dado do bundle foi de ~2,02→2,17 MB). Pisos por semana estáveis dentro de cada cultura (Grega 1,0–1,1 · Nórdica 1,0–1,7 · Egípcia 1,0–3,1 · Japonesa 0,7–1,0 · Chinesa 1,6–2,4).

**O QUE FALTA E DEPENDE DE ARMAZENAMENTO DURÁVEL QUE HOJE NÃO EXISTE (registro explícito, para quando a decisão voltar).** O placar **COMPARATIVO** (entre jogadores) e o **RANQUEADO** são, por definição, do servidor — e hoje o servidor não persiste: contas, apelido, pontos de ranqueado e progresso de missão vivem num `Map` em memória com write-through para `server/dados/contas.json`, disco **EFÊMERO** no Render grátis (`render.yaml` plano free, sem bloco `disk:`; §237/§274) — **cada deploy apaga tudo**. Para um placar semanal comparativo sobreviver, é preciso store durável — **as opções medidas na Fatia 2 (§274):** (a) Postgres gerenciado do Render, (b) disco persistente pago (a camada de contas já isola tudo em `_carregar`/`_persistir` — a reescrita é pequena), (c) um KV externo; commit-back ao git está descartado (dado de jogador, gitignorado). A **fronteira**: progresso PESSOAL = local (o que a Fatia 3 faz — funciona offline, por-aparelho); qualquer COMPARAÇÃO entre jogadores = servidor durável. Enquanto a decisão de custo não voltar, o modo fica no pessoal.

**O QUE A FATIA 3 NÃO TEM:** comparação entre jogadores, servidor, recompensa, rotação de trio.

**BABÁS** (`tests/dominios.test.js` + `perfil.test.js`): a virada de semana preserva o melhor de sempre e arquiva o recorde semanal; corrida em andamento não se perde na virada (carrega a sua semana); recorde anterior aparece e a superação é anunciada UMA vez; as escadas de todas as semanas (5×8) são monotônicas; **relógio alterado (adiantado E atrasado) não apaga progresso** (grava por MAX em chave, melhor de sempre só cresce). Perfil **v5→v6** (o recorde por-domínio vira `{melhorSempre, semanas:{}}`). Suíte + build verdes; dist verificado em viewport real (812×375 DPR2), fontes reais, **rede bloqueada**, 3 capturas (`docs/capturas-275/`): seleção com recorde da semana e anterior, o instante da superação, um Domínio na primeira semana (sem marca anterior).

**Arquivos:** `tools/gerar_dominios.js` (N semanas, pool reusado), `data/dominios/{grega,nórdica,egípcia,japonesa,chinesa}.json` (agora `semanas[]`), `src/dominios.js` (ISO-semana + `domEscadaSemana` + a corrida carrega a semana + superação), `src/perfil.js` (v6, `melhorSempre` + `semanas` por chave, clock-robusto), `src/ui/home.js` (recordes no cartão/hub + instante da superação), `src/shell.html` (CSS), `tests/{dominios,perfil}.test.js`, `ESTADO.md`, `CLAUDE.md`.

## §274 — DOMÍNIOS, Fatia 2: cinco Domínios + a tela de seleção. (Sem placar — o placar tem um portão de medição, respondido no fim.)

**AS CINCO ESCADAS (mesmo método, mesma régua do §273).** As cinco maiores culturas, cada uma um arquivo (`data/dominios/<cultura>.json`), gerado por `node tools/gerar_dominios.js --todas` (a config virou um bloco por cultura; método e régua IDÊNTICOS). Os trios foram escolhidos por três critérios, nesta ordem — **jogável** (sustain+dano+controle; três nukers morrem no nível 2), **autossuficiente** (nenhum dos três depende de um deus fora do trio — o Fujin do §271 é o contra-exemplo, e o gerador tem um cheque de dependência externa), **icônico** (os três rostos da cultura) — e MEDIDOS antes de aplicar:

| Domínio | cultura | trio | piso da gulosa (30 corridas) | rampa medida |
|---|---|---|---|---|
| Olimpo | Grega | zeus/poseidon/atena | **1,0** (min1 max1) | 0,00→1,00 |
| Asgard | Nórdica | odin/thor/loki | **1,0** (min1 max2) | 0,00→1,00 |
| Duat | Egípcia | ra/isis/osiris | **1,8** (min1 max4) | 0,00→1,00 |
| Takamagahara | Japonesa | amaterasu/susanoo/tsukuyomi | **0,8** (min0 max1) | 0,00→1,00 |
| Céus | Chinesa | sunwukong/**nezha**/nuwa | **1,2** (min1 max4) | 0,00→0,75 |

**O OUTLIER, medido e corrigido antes de aplicar (o dono pediu para saber).** A 1ª escolha da Chinesa — sunwukong/**guanyu**/nuwa — deu piso **3,7** (min2 max7) e rampa que só chegou a **0,58**: três deuses redondos formam um trio forte demais, então a escada fica fácil para a gulosa (vai fundo) e nunca endurece no topo — muito fora da curva dos outros quatro (0,8–1,8). Como o método tem de ser IDÊNTICO entre as cinco (nada de afinar `difTopo`/dano por cultura), a correção é o TRIO, não o parâmetro: **guanyu → nezha** (um pico de vidro no lugar do 3º deus redondo). Remedido: piso **1,2**, rampa 0,00→0,75 — de volta à curva, e mais icônico (o par Sun Wukong+Nezha da Jornada ao Oeste). Os outros quatro entram na curva sem ajuste. Piso baixo em todos é ESPERADO (§273: a régua não calibra este modo; a gulosa sacrifica um deus cedo, a mão humana vai fundo) — o valor da tabela é a COMPARABILIDADE entre Domínios, não o número absoluto.

**PROGRESSO POR DOMÍNIO, INDEPENDENTE (perfil v4→v5).** `perfil.dominios` deixou de ser uma run única e virou `{ porDominio: { <cultura>: { run:{...}|null, recorde:N } } }`. Correr no Olimpo não mexe em Asgard; `recorde` = nível mais fundo já alcançado naquele Domínio (nunca regride, mesmo quando a run zera). Migração v4→v5 move a run única do §273 (só existia Grega) para `porDominio.grega`. Continua RUN-SCOPED (a run zera com a corrida) e progresso PESSOAL LOCAL — não fere o invariante 3, e não é placar (que é comparativo → servidor; ver o portão abaixo). Porta de escrita única `definirRunDominio(perfil, cultura, run)`.

**A TELA DE SELEÇÃO (rota `dominios`) + o HUB por Domínio (rota `dominio`, params.cultura).** Ao entrar em DOMÍNIOS, os cinco em cartões (cultura, os três retratos, e o PROGRESSO daquele Domínio): **em corrida · nível N** (retomável, destacado), **recorde · nível R**, ou **não iniciado**. Uma corrida em andamento aparece no cartão e é retomável — abandonar sem querer e perder a corrida é o pior defeito de um modo de progressão, e a run persiste por-domínio a cada nível resolvido. Reusa a linguagem do §273 (cartões dourados, fichas de trio) — nada de padrão visual novo. Saída em todos os estados (‹ Início na seleção, ‹ Voltar no hub — §210) e o voltar do Android fecha o fim-de-nível para o hub do Domínio CERTO, corrida preservada (§240).

**O BANNER PROGRAMÁTICO SAIU (§213).** O cartão de `sharp` (gradiente + texto) do §273 ia para o carrossel da home ao lado de OITO ilustrações (o Fenrir, o Kukulkán, o trono do Odin) — um cartão programático no meio delas salta aos olhos, e o carrossel é a 1ª tela. Trocado pelo **placeholder do §213** (`placeholder:true` no `HOME_BANNERS` — fundo escuro + título em ouro, SEM `<img>`, logo sem 404). **A ilustração definitiva deve ter o nome `web/banners/dominios.webp`** (648×1008, como as outras oito) — o dono gera, como gerou as treze da campanha; ao chegar, tira-se `placeholder:true` e volta a `<img>`. **Como a build trata banner de home ausente hoje:** ela NÃO valida banner de home (só a arte de campanha por `_arteOk`); um `banners/<arte>.webp` ausente vira um `<img>` que dá 404 e o `onerror` remove para um rótulo-reserva — que é exatamente o 404 que o §213 evita. Por isso o placeholder (sem `<img>`) é o tratamento correto até a arte chegar.

**A PERSISTÊNCIA (medida, item 4 — sem construir; o portão do placar):** (a) num deploy hoje se perde TUDO do servidor — contas (token→conta), apelido, pontos de ranqueado e progresso de missão vivem num só `Map` em memória com write-through para `server/dados/contas.json` (`server/contas.js`), disco EFÊMERO no Render grátis (`render.yaml` plano free, sem bloco `disk:`; §237). (b) o ranqueado guarda os pontos nesse mesmo arquivo (`conta.ranque.pontos`, gravado por `aplicarResultadoRanqueado`) — há escrita em disco, mas o disco não sobrevive ao deploy (novo container do git, `server/dados/` gitignorado, cadastro renasce vazio). NENHUMA persistência do servidor sobrevive. (c) um placar semanal (comparativo → servidor) precisa de store DURÁVEL: Postgres gerenciado ou disco pago no Render (a camada de contas já isola tudo em `_carregar`/`_persistir` — a reescrita é pequena), ou um KV externo; commit-back ao git está descartado (dado de jogador, gitignorado). (d) o progresso PESSOAL de Domínio já vive local (`localStorage['incursion:perfil']`, sobrevive a restart, por-aparelho) e funciona offline — a FRONTEIRA é essa: progresso pessoal = local (o que a Fatia 2 faz); qualquer COMPARAÇÃO entre jogadores (um placar) exige o servidor durável. Sem placar nesta fatia.

**BABÁS** (`tests/dominios.test.js`): os cinco aparecem na seleção e cada um abre o SEU; progresso independente por Domínio; corrida retomável e nunca perdida ao sair da tela; as cinco escadas monotônicas (como a grega); nenhum banner de home renderiza `<img>` 404 (Domínios é placeholder). Migração v4→v5 e independência em `tests/perfil.test.js`. Suíte + build verdes; dist verificado em viewport real (812×375 DPR2), fontes reais, **rede bloqueada**, 3 capturas (`docs/capturas-274/`): seleção com progressos diferentes, hub com corrida em andamento, entrada de um Domínio novo.

**Arquivos:** `tools/gerar_dominios.js` (por cultura + `--todas`), `data/dominios/{grega,nórdica,egípcia,japonesa,chinesa}.json`, `src/perfil.js` (v5, `porDominio`, `definirRunDominio`), `src/ui/home.js` (seleção + hub + banner placeholder), `src/view.js` (rota `dominio`), `src/shell.html` (CSS seleção), `web/banners/dominios.webp` (REMOVIDO), `tests/{dominios,perfil,render_sweep,aquisicao}.test.js`, `ESTADO.md`, `CLAUDE.md`.

## §273 — DOMÍNIOS, Fatia 1: a CORRIDA jogável (um Domínio, sem placar e sem servidor). A medição da fase 1 desenhou o modo; esta fatia o construiu.

**O QUE É.** DOMÍNIOS (do livro do dono: "atravessá-lo exige dominar os Domínios do Uno — cada um alimentado pela fé de um povo") é um modo roguelike: **três deuses definidos, iguais para todos** (sem montar time — competição de skill, não de coleção), que **descem uma ESCADA** de batalhas 3×3. A **vida CARREGA** entre níveis com cura parcial; a cada 10 níveis um **CHEFE** (trio de uma cultura só, sinérgico por construção — o espelho do que o jogador faz); vencido o chefe, um **PRÊMIO** (cura / reviver um caído / bônus de dano). **Fatia 1 = UM Domínio** (o do Olimpo, trio zeus/poseidon/atena). SEM placar, reset semanal, rotação, servidor ou recompensa — só a corrida, progresso local no perfil. Se a corrida não diverte, placar nenhum salva, e o caro é o servidor.

**O QUE A MEDIÇÃO (fase 1) DERRUBOU, e virou desenho:**
- **O SORTEIO saiu.** Com vida cheia, o melhor trio ainda perde ~6% dos sorteios (exp 4) — sorteio livre é loteria (a corrida acaba por azar, não por desgaste), e sorteio de semente compartilhada faz todo mundo morrer no mesmo muro. Nenhum dispersa. **No lugar: uma ESCADA MEDIDA, gerada fora do jogo e publicada como DADO** (`data/dominios/<cultura>.json`), em RAMPA — os fáceis embaixo, os duros fundo. Escada nova no futuro = arquivo novo (`tools/gerar_dominios.js`), **nunca código novo**.
- **A CURA não é a alavanca (para trio forte).** A medição mostrou a cura quase sem efeito na profundidade quando o sorteio domina (exp 3). Só depois de **restringir o sorteio** (a escada bane os hard-counters — `capComum 0,45`) a cura passa a valer. Começou em **25/nível**; é knob de dado, o dono calibra.
- **A dificuldade sobe por DANO do inimigo, não por VIDA.** Inflar vida reduz a profundidade mas quase dobra os turnos (16→28): fica mais LONGO, não mais difícil. Dano endurece sem alongar (exp 2). Rampa suave por faixa de 10: **+0% / +5% / +10% / +15%**.
- **O TETO do bônus de dano é +50%.** Acima, a corrida explodia para 23+ e batia no teto de níveis (exp 5). O prêmio de BÔNUS soma +10% por chefe, teto **+50%** (cinco escolhas); no teto, a opção BÔNUS **some** (§252: opção que não faz nada é armadilha — a mesma regra do REVIVER sem caído).

**A FORMA DA RAMPA (a escada do Olimpo, com números).** 40 níveis, 4 faixas, 4 chefes. A régua é o experimento 4: `dificuldade[n] = 1 − (vitórias do trio GULOSO do jogador, vida cheia, contra os inimigos do nível, no danoMult da faixa; 12 seeds)`. Comuns escolhidos para um ALVO de dificuldade-base subindo linear 0→0,45 (hard-counter > 0,45 fora do pool); o DANO da faixa soma por cima. Resultado **monotônico, 0,00 no nível 1 → 1,00 no nível 40** (chefes Brasileira n10 · Maia n20 · Chinesa n30 · Hindu n40, em dureza crescente). A build valida a monotonia (`domValidarLadder`, tolerância 0,06 = ruído medido da régua) e falha-alto se a escada regredir.

**O PISO DA IA GULOSA nesta escada = 1** (30 corridas, política de prêmio "reviver caído senão curar": med/min/p50/max todos 1). É o número que faltava, o piso contra o qual o dono compara a mão dele. Baixo de propósito: **a régua não calibra este modo** (a fase 1 já provou — mediana 0 em 6 das 10 culturas) — a IA gulosa SACRIFICA um deus já no nível 1 (0,00 de dificuldade a vida cheia, mas a vitória custa uma peça), e entra no nível 2 em desvantagem de HP. A mão humana não faz isso; a dispersão é gulosa-morre-cedo × humano-vai-fundo, exatamente o pedido.

**AS DUAS DECISÕES NOVAS (regras da corrida):**
1. **"1× por partida" que RESSUSCITA vira "1× por CORRIDA".** Revive/vidaExtra/auto-renascimento (os 17 medidos) — rede que se remonta a cada nível não é rede, e competiria com o prêmio do chefe. O resto do "1× por partida" (picos de dano, únicos como o Terceiro Olho) fica **POR NÍVEL** — é o pico do kit, e um pico que dispara 1× em 40 níveis deixa de existir. **Implementação (regra dura: fora de Domínios NADA muda):** um flag dedicado `reviveGastoCorrida` na unidade, **default false**, checado nos TRÊS revive-sites do motor (vidaExtra em `matar`, `reviveProximoTurno`, `reviver`-por-aliado). A camada de corrida (`src/dominios.js`) detecta o revive no log e carimba o flag por-deus no próximo nível. O "por nível" é o DEFAULT grátis: cada nível é um `novoEstado` fresco (`usos={}` zera).
2. **CARREGAM: vida e bônus. NÃO carregam: recarga, efeitos, contadores.** Recarga que carrega premia matar rápido, e matar rápido já é premiado pela vida que sobra — os dois juntos fariam existir uma estratégia só. Também default grátis: `novoEstado` fresco por nível zera cd/efeito/contador; a corrida só re-injeta hp+vivo (por slot) e escala o dano do trio pelo bônus (via catálogo, como a Provação passa o seu). Passivas de ABERTURA disparam por nível, como sempre (identidade de kit, igual para todos).

**INVARIANTE #3 (sem progressão permanente) — respeitado.** Tudo é da CORRIDA e ZERA com ela: vive em `perfil.dominios.run` (blob run-scoped, `run:null` encerra — o "reset por fronteira" do sandbox), **nunca** toca `deuses`/kit/HP-base. Perfil v3→v4 (backfill vazio). O bônus de dano é poder run-scoped que evapora, não poder comprável nem carregável entre corridas.

**TELA (§210/§240).** O HUB (`rota 'dominios'`, 9º banner da home) é a entrada, o status da corrida e a **escolha de prêmio** — que vive no hub, não como overlay órfão de batalha: assim tem saída (‹ Início, §210) e o VOLTAR do Android fecha a sobreposição de fim-de-nível para o hub ANTES de qualquer coisa, com a corrida PERSISTINDO (aguardandoPremio) para retomar (§240 — sem beco sem saída). REVIVER só aparece com caído; BÔNUS só abaixo do teto; CURA é o piso.

**BABÁS** (`tests/dominios.test.js`): (1) o gate `reviveGastoCorrida` — com ele a Vida Extra não ressuscita, sem ele sim, default false fora de Domínios; (1b) recarga/pico/efeito/contador ZERAM por nível; (2) vida e bônus carregam, e o bônus escala só o trio e o danoMult só o inimigo (não cruzam); (3) REVIVER só com caído; (4) o bônus não passa de +50% e some no teto; (5) a escada é monotônica; (6) §210/§240 no dist (fim de nível tem saída, o voltar fecha para o hub, a corrida persiste). Suíte + build verdes; dist verificado em viewport real (812×375 DPR2), fontes reais, **rede bloqueada**, 5 capturas (`docs/capturas-273/`): entrada, nível comum, chefe, prêmio com e sem caído.

**Arquivos:** `src/dominios.js` (motor da corrida), `tools/gerar_dominios.js` + `data/dominios/grega.json` (escada medida), `src/engine.js` (flag `reviveGastoCorrida` + 3 gates), `src/perfil.js` (v4, `dominios.run`, `definirRunDominios`), `tools/build.js` (valida+injeta DOMINIOS, concatena dominios.js), `src/ui/home.js` (HUB/HUD/reconciliador/overlays/banner), `src/view.js` (rota + despacho + voltarNativo), `src/ui/sobrepor.js` (limpa latch no Sair), `src/shell.html` (CSS), `web/banners/dominios.webp`, `tests/{dominios,perfil,render_sweep,aquisicao}.test.js`, `package.json`, `ESTADO.md`, `CLAUDE.md`.
...
**1. Os 5 metadados reconciliados (o motor lê faccao/elem/classe/funcao).** afrodite/apolo **Olímpica→Grega** (não é uma das 10 mitologias; o código de missões já a tratava como Grega); exu **Brasileira→Africana** (orixá com orixás); kraken **Grega→Nórdica** no kits.json (lenda escandinava; a associação grega é de cinema); hermes **Suporte→Manipulador** (LI O KIT: rouba orbe, arranca buff, manipula recarga — não cura, não protege, não dá orbe; precedente do Exu). **Único com combate vivo:** hermes, porque **Aquiles sofre +10 de Manipuladores** (`deFuncao`) — mas medido: **Hermes e Aquiles nunca ficam em lados opostos** em nenhuma Provação/Semanal/campanha, então **zero veredito muda**. kits.json volta a ter exatamente **10 facções, soma 100**. Re-carimbo (§263, sem re-resolver): 13 Provações refrescaram o hash, **nenhum veredito se move**.

**2. O guarda de sincronia (falha-alto na build), muito maior que os números.** Antes só dano e cura eram conferidos — 2 dos 6 eixos. Agora o `checar_cadeia` também confere: **os metadados** (faccao, elem↔elemento, classe↔tipo, funcao — o buraco exato dos 5) e **três eixos de valor**: `orbe`, `escudo`/Defesa, `combo` (geração), ESTRITOS; e **`duração`** com a **convenção do agendado codificada** — medido: dos 55 descasamentos, 1 é a convenção (Kukulkán: efeito agendado conta a vez da aplicação, fx=texto+1), 54 são ambiguidade de texto (multi-status), **0 defeito real**; a regra estrita vale 1×1, o resto é não-conferível. **Babá por eixo** (`cadeia.test.js`): mexe o fx sem tocar o texto → quebra alto.

**O QUE FICOU DE FORA DO GUARDA, e por quê (o risco não é o buraco, é achar que cobre tudo):**
- **eixo `buff` (+/− de dano): FORA.** A magnitude mora em ≥5 tipos de efeito (bonusDano/reducao/vulnerabilidade/dmgUp/dmgDown/dmgReduction) e o texto "+N de dano" não distingue buff-próprio de debuff-no-inimigo de redução (Aquiles: 12 de redução + 10 de vulnerabilidade num só texto). Guarda estrito mentiria.
- **os 5,4% não-conferível** (91/1683): dano multi/condicional (o valor-por-golpe mora no motor, a contagem na prosa — §92), fx dinâmico (alterna/opcoes), duração multi-status. São REPORTADOS, nunca falha-alto — o parser não resolve com segurança e prefere calar a mentir. Se a proporção subir, o teto de 20% do `cadeia.test.js` quebra (padronizar a prosa, não afrouxar o parser).

**A CADEIA COBRE 2 DOS 4 CATÁLOGOS — buraco medido e registrado (§271, risco do §250: o marcador que sobrevive ao motivo).** O `checar_cadeia` confere **kits.json ↔ data/deuses**. Ele **NÃO confere `roster_data.js`** (o 4º catálogo, que alimenta o seletor, a maestria por facção e a composição) **nem o texto de outras telas que não a Coleção**. Uma sessão futura vai ler "cadeia conferida" e supor cobertura total — não é. **Deriva do roster medida (roster ↔ data/deuses, 100 deuses × 5 campos): 1 divergência — `kraken.faccao` "Grega" no roster, "Nórdica" no motor.** Consumidores AINDA ativos do `roster.faccao`: `selecao.js` (filtro/contagem e legenda do card), `home.js` (maestria agrupada por facção, composição), `campo.js` (fallback do rótulo em batalha) — nesses, o Kraken ainda cai como "Grega". A Coleção (detalhe) foi desviada para GODS (§271 item 3); alinhar o roster ou colocá-lo sob a cadeia é decisão do dono, medida e anotada, NÃO consertada aqui.

**3. A Coleção lê o `.desc` e os metadados do `data/deuses`.** A auditoria achou 58 textos divergentes coleção↔batalha, **9 substantivos**, TODOS a coleção OMITINDO (odin perdia o "+6 contra marcados", susanoo o Combo, boto a Noite). Conserto: a Coleção (`renderDeusDetalhe`) passa a mostrar o `.desc` (o texto do motor) e os metadados de `GODS` — **fonte única, a deriva de texto acaba na origem**. **kits.json deixa de ser fonte de TEXTO DE TELA, mas CONTINUA sendo o lado esquerdo da cadeia** — a intenção humana que a build confere contra a máquina (números + metadados + eixos). Antes de trocar, varri os 100 `.desc`: o único vazamento de texto interno era `fujin.passiva` ("INERTE neste protótipo") — resolvido no item 4; re-varredura pós-fix = **0 notas de dev**. (ROSTER segue como fonte para nome/retrato e outras telas — reconciliá-lo é auditoria futura, anotada.)

**4. Fujin: o 5º slot morto, implementado (a medição contradisse "o mecanismo está pronto").** O Fujin é inicial e aliado fixo do Prólogo IV, com passiva `fx:null` — **morto para quem acaba de instalar**, e o pior dos 5 (os 4 do §189 são fracos; este não existia). A intenção ("com Raijin no time, ambos geram +1 de Combo") é o mecanismo do Raijin (`geraContadorPorGolpe`) com uma condição. **Mas não compunha:** o combo é pool-por-LADO, e nem o `porTurno` (escreve o store por-unidade) nem o `geraContadorPorGolpe` (sem gate) alcançavam a condição sem código. **Decisão do dono:** gatear `geraContadorPorGolpe` por `estado` — vocabulário existente virando condicional, não código para um deus. **Regras duras respeitadas:** `estado` ausente = incondicional (Raijin **intacto**, o caso medido); a avaliação usa o **mesmo `estadoOK`** do resto do motor (§266: uma só conta, indicador e efeito não divergem); entrou no VOCAB. Fujin: `{gatilho:'geraContadorPorGolpe', contador:'combo', v:1, estado:{aliadoPresente:'raijin'}}`. Medido: **Fujin e Raijin nunca ficam juntos** em conteúdo nenhum → **inerte no conteúdo atual, zero veredito**; o re-carimbo do Fujin (a única Provação dele, sem Raijin) não move veredito. **P do §266 acende para o Fujin só com Raijin em campo** (legível, não só funcional). **Babá** (`capacidades.test.js` §271): Raijin gera sem estado; gate genérico com/sem o aliado; Fujin gera e é legível só com Raijin. **DEFEITO DE DESENHO registrado, não consertado:** passiva de deus INICIAL que depende de deus NÃO-INICIAL é morta para o novato — revisão de kit da Fase 4, com medição própria (ESTADO.md, slots mortos).

**Arquivos:** `data/deuses/{afrodite,apolo,exu,hermes,fujin}.json`, `data/kits.json`, `data/provacoes/*` (14 re-carimbos), `src/engine.js` (estado-gate + infoPassiva), `tools/checar_cadeia.js` (metadados + eixos), `tests/{cadeia,capacidades}.test.js`, `src/ui/home.js` (Coleção lê GODS). Suíte + build verdes.

## §269 — DUAS CORREÇÕES nos atos de escolha do Cap 1 (o prêmio vazio do Nezha; a opção sem rosto).

**1. O prêmio do Nezha estava vazio — leitura sem recompensa não é leitura.** O Nezha é um dos 9 fixos do `novoPerfil`, e o slot emprestado do ato VI já é trocável por qualquer deus que o jogador tenha. Então "acertar traz o Nezha" dava o direito de usar um deus que ele já podia escolher — mecanicamente **zero**, e o princípio do §268 é: se nada se ganha, nada foi lido. **O conserto estava na própria cena:** o Nezha é a **testemunha** (passou o ato V escondido, olhando) — o que uma testemunha dá é o que ela **viu**, não a espada dela. Mantido o empréstimo do Nezha (a revelação segue verdadeira, valor narrativo), **acrescentado `kitRevelado` do Fenrir**. O esquema **aceita dois verbos no mesmo `efeito.certa`** (`{emprestado, kitRevelado}`): o validador itera as chaves da consequência e `kitReveladoDaConsequencia` já devolve lista — dois chips renderizam. **O desenho que fecha (vale registrar):** Conselho certo mostra a **Hel**, Nezha certo mostra o **Fenrir**; as duas leituras certas e o jogador entra no clímax conhecendo os dois; nenhuma e ele enfrenta o pacto **às cegas** — que é literalmente o que Zeus faz na história. Cena de leitura paga em informação, sempre na mesma moeda.

**2. A Hécate sai, o Exu entra (decisão do dono).** A Hécate **não existe no jogo** (sem kit, sem arte) — era a única opção do Conselho **sem retrato** (aparecia como o monograma "HÉ"), e num ato de LEITURA opção sem rosto é **dica involuntária** (a estranha salta aos olhos). Substituída pelo **Exu** (chave `exu`, com kit e retrato): cumpre as três funções da cena — a **encruzilhada** (o domínio dele), a **multiplicidade** (os muitos Exus, um por caminho, no lugar da triplicidade) e o **riso de quem já sabe o fim**; e a passiva ("age primeiro em todos os turnos") caracteriza a cena sem uma linha de código. No dado: a opção passa a `deus:"exu"` (as 4 do Conselho agora têm rosto), a pista vira a do Exu (mantendo a função de isca — o barulhento; a certa segue Ah Puch, o silencioso), e o texto do ato troca a linha da Hécate pela do Exu, mantendo a ordem dos quatro à mesa. Docs atualizados: `campanha-prologo-cap1.md` (texto do ato II + a seção "pendente de propósito", que dizia que ela precisava de retrato e não de kit — deixou de valer) e `campanha-ato-escolha.md` (tabela das 4 opções). Os `.docx` do dono NÃO foram tocados.

**GUARDA (`campanha.test.js` §269):** (A) toda chave `deus` numa opção resolve a um deus real (a Hécate era nome sem chave — proibido); as opções de um ato são **homogêneas em retrato** (todas com rosto ou nenhuma — mista deixa a sem-rosto virar dica); o Conselho tem as 4 com rosto. (B) o prêmio do Nezha revela o kit do Fenrir; as duas leituras certas dão dois chips (Hel+Fenrir); nenhuma dá zero. Suíte verde.

---

## §268 — O ATO DE ESCOLHA (terceiro tipo de ato). Motivo: um segundo jogador estranhou que atos de história não fossem jogáveis — 3 em 13 hoje, uns 30 nos 22 capítulos, e o pior é o 1º ato do jogo (a única ação é ler e apertar continuar).

**O princípio (de docs/campanha-ato-escolha.md, commit 70479d7 — a FONTE; nada foi inventado além do que abaixo se declara).** O ato de escolha é **a competência central do jogo aplicada fora do combate**: o INCURSION cobra ler o oponente; conselho e testemunha são exercícios de leitura que não estavam sendo jogados. **Tem resposta certa** (vem do livro, não da mecânica). **Muda a MECÂNICA, nunca a narrativa** — decide quem você leva, quantos orbes começa, o que sabe do inimigo; a história segue linear (se ramificasse o texto, 130 atos virariam 300 — alternativa RECUSADA). **Errar custa, nunca bloqueia.** **A consequência se revela quando acontece**, não na hora da escolha.

**FASE 1 (o portão de medição) contradisse o plano — parei e reportei.** O plano pedia a consequência `orbes menor` no Conselho errado, a medir contra o clímax (ato VI do Cap 1, 29,5% guloso). Medindo: (a) a linha-base de 29,5% não se reproduziu (36,1% jogador-abre / 24,3% alterna / 14,5% inimigo-abre — variação de quem abre, não medida no plano); (b) o baseline de orbes é **0** (o `orbes menor` não tem o que tirar); (c) sob IA gulosa o handicap de orbe é **não-monotônico** (às vezes ajuda). Nunca cruzou o injogável, mas era irrepresentável e ruidoso. **Decisão do dono:** Conselho errado = **sem `kitRevelado`** (custa o bônus de informação, risco de balanço zero). Consequência: **NENHUMA BATALHA MUDA** em lugar nenhum; `orbes` fica no vocabulário (a build o aceita) mas nenhum ato o usa.

**O esquema (`tipo:"escolha"`).** Campos: `pergunta`, `opcoes[]` (`{id,rotulo,deus?,pista?}` — `deus` é retrato quando a opção É personagem), `certa` (OPCIONAL), `alvo` (a batalha onde a consequência cai), `efeito` e `revelacao`. **`efeito`/`revelacao` são indexados pelo RESULTADO:** com `certa` → `{certa,errada}`; **sem `certa`** (o ato de identidade) → uma entrada **por opção**, e NUNCA `errada` (não há leitura errada quando nada se lê). **Vocabulário de consequência FECHADO em três: `emprestado` | `orbes` | `kitRevelado`.** Fora dele — em especial **acrescentar/remover inimigo** — a build RECUSA (muda o balanço medido do alvo sem remedição). A validação mora em `tools/valida_campanha.js` (a build E o teste chamam a MESMA função).

**Os três atos convertidos (batalhas intactas — aliados/inimigos/seed no dado ficam iguais).**
- **Prólogo I (A Era dos Planos)** — identidade, **sem `certa`** (é o único do tipo: antes da fusão não há quem ler). 4 planos (Olimpo/Asgard/Duat/Xibalba); consequência `emprestado` no **Prólogo VI** (o único ato em que o time é do jogador). **Autorado, sinalizado (não estava na fonte, que nomeia os planos mas não os deuses):** o mapa plano→deus — Olimpo→**poseidon**, Asgard→**thor**, Duat→**osiris**, Xibalba→**kukulkan** — e as 4 pistas de plano + as 4 linhas de revelação. **CRITÉRIO (correção, ver abaixo):** cada plano empresta um deus **fora dos 9 fixos do `novoPerfil`** — emprestar quem o jogador já tem garantido não é anzol. Baixo risco: o emprestado é **trocável** no ato VI (risco nenhum).
- **Cap 1 II (O Conselho Temporário)** — o enigma; **conteúdo 100% da fonte** (pistas verbatim). `certa=ahpuch` (o silencioso que vende o segredo — "ler o kit, não o rótulo"). Certo → `kitRevelado` de **Hel** no ato VI; errado → `{}` (sem kit). Revelações verbatim. (Hécate é opção sem retrato — não é deus jogável no acervo; mostra o monograma "HÉ".)
- **Cap 1 V (O que Nezha viu)** — a leitura mais limpa (o jogador acabou de ser o trovão no ato IV). `certa=rivais`; certo → `emprestado` **Nezha** no ato VI; errado → `{}` (o ato já carrega a consequência do Conselho; duas punições empilhariam desvantagem sem medição). As 3 opções são conceitos (sem retrato, sem pista — a evidência foi VIVIDA). Revelação certa verbatim ("Nezha viu o mesmo que você.").

**Runtime (a consequência nunca toca o dado da batalha).** `consequenciasNoAto(alvo)` varre as escolhas gravadas (`perfil.campanha.escolhas[atoId]=opcaoId`) que apontam para o alvo e resolve o resultado. `emprestado` **semeia o default do 1º slot emprestado** (trocável; `campSwap` do jogador vence) — em Prólogo VI (aliados:null) semeia o slot 0 e deixa 2 vazios. `kitRevelado` vira um **chip que abre um overlay** com as 4 skills (não cabe inline nos 290px sem cortar, §262). A `revelacao` aparece no ato-alvo (o jogador liga a linha ao efeito sozinho). **Escolher SELECIONA; CONFIRMAR grava e avança — sem volta** (escolhi seleção+confirmar em vez de um-toque-confirma para dar o instante de rever a pista; "sem volta" é depois de confirmar).

**A trilha distingue TRÊS formas de nó (sem rótulo escrito — o nó é pequeno; escolhi o sinal): batalha = losango cheio · história = círculo/anel · escolha = losango com um EIXO diagonal ciano (a encruzilhada).** O sinal fica em `transform`/`::after`, que os estados feito/atual (fundo, tamanho) não tocam — sobrevive a vencido e atual. **Nota:** os 3 atos de história de hoje viraram escolha, então `historia` tem **0 instância** no jogo atual (a 3ª forma é para capítulos futuros com narração pura; o teste prova as 3 formas com um nó de história injetado em memória).

**Guardas.** Build recusa (via `valida_campanha.js`, testado com dado inválido): verbo fora dos 3, `efeito.errada` sem `certa`, `kitRevelado` de quem não é inimigo do alvo, alvo que não é batalha. `campanha.test.js` §268 A/B/C/D: rejeição da build; erro nunca tira orbes (não bloqueia); a consequência cai certa no alvo (empréstimo/kit/revelação, e nada quando nada foi escolhido); as 3 formas de nó distinguíveis. Suíte verde. Verificado no dist (viewport 951×460 DPR 2, fontes reais, **rede bloqueada: 0 requisições externas**), 4 capturas na sessão (Conselho com 4 opções+pistas; ato VI acerto com revelação+chip+kit de Hel; ato VI erro sem kit; trilha com as 3 formas) — como as capturas anteriores, não versionadas.

**CORREÇÕES (mesma sessão, três achados do dono).**

1. **O mapa plano→deus anulava o mecanismo em duas opções.** Os 9 iniciais são LISTA FIXA no `novoPerfil` (`zeus/ogum/tyr/sobek/brigid/ganesha/cuca/fujin/nezha`), garantidos no turno zero — E o `grantInicial` dá 1500 gemas (um pacote de 10). Logo **zeus e sobek são posse garantida** (e o zeus é com quem se jogou 6 atos do Prólogo): Olimpo e Duat emprestavam quem o jogador já tinha — anzol morto. Trocados: **Olimpo→poseidon** (Olímpico icônico, não-inicial), **Duat→osiris** (o senhor do Duat, não-inicial). Critério gravado no dado: **"fora dos 9 fixos do `novoPerfil`"** — o pacote de 10 impede garantia ABSOLUTA de não-posse, então essa é a linha acionável. **Eco narrativo não planejado (gravado no dado):** quem escolhe o Duat recebe **Osíris, o Rei dos Mortos**, e 4 atos depois vê Zeus devastar o Duat no Cap 1 IV ("nenhum deus egípcio ousou mais emergir") — a escolha ganha consequência narrativa sem código. **Exceção deliberada:** o empréstimo de **Nezha** (Cap 1 V, ato COM `certa`) é inicial, mas fica — ali o empréstimo é recompensa NARRATIVA da fonte (o protagonista dos 22 capítulos aliando-se a você por ter lido certo), não um anzol de acervo. O guard do critério cobre só os atos de identidade (sem `certa`).

2. **O `_seedNota` gravava números que não reproduziam.** As 4 batalhas fixam `comeca:0` (jogador sempre abre), mas os números gravados vinham de método não registrado. Re-medi com `tools/medir_campanha.js` (permanente, não descartável): guloso×guloso, jogador abre, N=2000 sementes 1..N, determinístico, `repro:` gravado em cada nota. **Antigo→novo:** cap1-i 100→**99,6%**, cap1-iii 43→**59,2%** (errado), cap1-iv 90→**88,2%**, cap1-vi 29,5→**36,1%** (errado — foi ESTE número, planejado como base da FASE 1, que motivou tudo; o 36,1% reproduz a medição da FASE 1). Cada `_seedNota` agora diz que a taxa é **PISO** (IA gulosa dos dois lados; o humano joga melhor, a dificuldade real é menor) — sem isso, uma sessão futura lê 36,1% e conclui "quase impossível", o erro que o próprio dono cometeu.

3. **`orbes` e `historia`: consumidor sem produtor, DELIBERADO (§95, mas o inverso do §250).** Ambos ficam, guardados por teste sintético, e o **motivo fica escrito aqui** — o risco não é o peso do código morto, é alguém "arrumando a casa" e apagando o que ainda serve (foi o inverso do cinza do PvP do §250, que sobrou sem servir; o antídoto é o mesmo: o motivo onde a próxima sessão lê). **`orbes`** é a **válvula medida da FASE 1** — o único verbo que altera balanço; fica no vocabulário para o dia em que uma medição mostrar que um ato precisa dele; guard sintético exercita o ramo do validador (número passa, não-número recusa) sem ato real. **`historia`** é um TIPO de ato que hoje tem **0 instância** (os 3 viraram escolha); **volta no Capítulo 2** quando houver cena de narração pura; a checagem inline saiu do `build.js` para `tools/valida_campanha.js` (`errosHistoria`) para ficar testável sem ato real. Ambos com babá em `campanha.test.js` (§268 A/A-bis). Suíte inteira verde.

## §267 — DUAS LACUNAS DE SIMETRIA DO §266 (coerência do indicador, não features novas). Medi as duas; consertei só a que a medição confirmou.

**LACUNA 1 — a leitura em repouso ainda mente? NÃO (sem conserto).** Medido no dist: com nada armado, tocar o "P" do Ogum mostra **"PARADA AGORA (a condição não vale no momento)"** + o texto do kit ("+10 de dano contra alvos com escudo ou redução…"). O "SEMPRE ATIVA" que mentia foi removido no §266; a leitura em repouso diz PARADA e o desc nomeia a condição. Não mente → nada a consertar (o critério do dono era "conserte só se mentir").

**LACUNA 2 — a redução do defensor acendia sem casar o golpe? SIM (confirmado, consertado).** O §266 fez as 17 passivas SÓ-ALVO do ATACANTE acenderem só ao mirar o alvo que casa. A simetria defensiva faltava: em `_fxAtivo`, a `reducao` retornava sempre que `v>0`, ignorando o `contra`. Medido: sobek (contra=`básico`) acendia o P **em repouso** E com uma **habilidade** mirada (que o `contra` não casa) — o indicador enganava, dizendo que reduz ESTE golpe quando não reduz (exatamente a opção 3 que o dono recusou). São **7 reduções com `contra`** (todas self): afrodite/bastet/hermes/iara (`alcance:único`), sobek (`slot:básico`), oni (`classe:Mágico`), baldur (`elemNão:Verdejante`); as outras **11 não têm contra** (redução permanente).

**CONSERTO (simetria):** `infoPassiva` passa a receber o GOLPE mirado (`{slot,classe,elem,unico}`, montado em `_armadoCtx` a partir da ação armada); `_fxAtivo` gateia a `reducao` com `contra` por `contraCasou(f.contra, golpe)` — acende só quando o golpe mirado casa o filtro. Sem `contra` = redução PERMANENTE, de pé mesmo em repouso (como a aura ofensiva incondicional da Brígida). Provado: sobek acende com BÁSICO mirado, apaga com HABILIDADE; afrodite acende com golpe ÚNICO, apaga com ÁREA; poseidon (sem contra) de pé em repouso. (Descoberta na captura: sob a aura de time do Poseidon, todo inimigo acende — correto, é a redução de time incondicional caindo sobre eles; a contra-redução PRÓPRIA do sobek gateia certo por baixo disso.)

**GUARDA:** engine (`capacidades.test.js` §267): sobek parado em repouso, acende só no básico mirado, apaga na habilidade; afrodite único×área; poseidon permanente de pé. UI (`moldura.test.js` §267): no dist, o P do sobek acende ao armar básico e apaga ao armar habilidade. Suíte verde.

## §266 — INFORMAÇÃO DOS MODIFICADORES DE DANO: o motor estava CERTO e a tela estava MUDA. Motivo: o dono, jogando, viu um básico bater acima do escrito e — o AUTOR do jogo — não conseguiu explicar por quê. Num jogo cuja tese é INFORMAÇÃO COMPLETA (nenhum atributo oculto), isso é defeito — de tela, não de conta (a auditoria provou 184/189 golpes aplicam o escrito).

**FASE 1 — medido antes de construir.** (a) Pior caso 3×3: até **6 fontes de modificador num só golpe** (base 15 → 56): 3 passivas de aura (Brígida +5, Mímir +6, Apolo +6 vs HP cheio) + 3 efeitos (dmgUp, vulneravel, adormecido). (b) O talo de efeitos é 92×18px, cabe **5 chips de 14px** (o 6º vira "+N"); a magnitude não cabe inline sem cair pra 4 chips. (c) O "P" só distinguia tem-passiva/inerte/sem — **não** agindo-agora; e a leitura mentia "SEMPRE ATIVA" (falso para as condicionais). (d) **39 passivas são avaliáveis em repouso** (incondicional + campo/self); **17 são SÓ-ALVO** (só dá pra saber ao mirar). Achado extra grave: **`vulneravel` não estava no SYM** → o debuff "+N de dano recebido" era **invisível por completo** (sem chip).

**FASE 2A — a passiva se anuncia quando age.** Motor novo `infoPassiva(st, u, armado)` (PURO, reusa os mesmos gates da conta de dano — o P nunca acende uma mentira): diz o que a passiva de `u` modifica AGORA e quais auras de aliados caem sobre `u` (com a FONTE). O botão **"P" ACENDE** (`pas--on`, preenchido dourado + brilho) quando a passiva está modificando algo; fica no contorno quieto quando existe mas não age. Decisão do dono: as SÓ-ALVO (Ogum +10, Apolo +6) **acendem ao ARMAR o golpe e mirar um alvo que casa**. A **aura que afeta o time é legível a partir do deus AFETADO** (não só da fonte): sob a Brígida, os 2 aliados acendem, e tocar o P deles mostra "**+5 de dano — de Brigid**". Tocar o P leva ao rodapé com "**AGINDO AGORA: <itens com valor>**" + o texto da passiva. Provado na tela: Ogum armado no alvo com redução → P aceso + "**+10 de dano (neste alvo) · fura redução**" — o **25 vira explicável sem toque de adivinhação** (15 base + 10 passiva, e fura a redução do alvo).

**FASE 2B — os chips ganham magnitude.** `vulneravel` entrou no SYM (glifo ⊕) — deixou de ser invisível. Os chips são **adaptativos**: com **≤3 efeitos** crescem e mostram o NÚMERO (o **+8 do adormecido**, o +v do vulnerável/dmgUp, o −v da redução), com **4+** colapsam pro chip de 14px + turnos (número no toque) — assim o pior caso de 6 continua cabendo no talo de 92px. O adormecido mostra **+8** (constante do motor, que o dono pediu explícito).

**O QUE NÃO FOI FEITO (fora do escopo, decisão do dono):** NÃO se recalcula o texto do kit na ficha nem no rodapé (número efetivo por alvo é outra decisão, não aprovada); NÃO se mexeu na conta de dano (está correta, §262-auditoria).

**GUARDAS (babá):** engine (`capacidades.test.js` §266): Brígida aura legível no afetado (+5 e fonte) · Ogum parado em repouso, age ao mirar o defendido · Amaterasu acende só no Dia. UI (`moldura.test.js` §266, fontes reais): P aceso sob aura nos 3 afetados e apagado no inimigo (agindo distinguível de parado) · a leitura do afetado traz +5 e a fonte · o chip do adormecido mostra +8 sem toque · o talo com 6 efeitos não estoura. **Capturas no dist** (viewport real, rede bloqueada): Ogum 25 explicável, adormecido +8 visível, Brígida acendendo os 2 aliados, 6 efeitos cabendo. Suíte inteira verde.

**INVARIANTE (CLAUDE.md):** um modificador que altera número tem de ser legível na tela SEM TOQUE.
...
**O QUE PASSA A COBRIR:** só o que o MOTOR lê (combate). Introduzi `projecaoCombate(g)` e o hash passa a rodar sobre ela. **CRITÉRIO:** um campo é EXCLUÍDO só se for rótulo de APRESENTAÇÃO NAQUELE nível que o motor nunca lê. A deny-list é **por nível, não por nome-de-chave** — porque `nome` DENTRO de um `fx` é a CHAVE de contador/dot que o motor lê (`getContador(u, e.nome)`, `dots.find(d=>d.nome===…)`): combate, fica. Derivei os INCLUÍDOS da lista que o motor realmente consome (conferido no `engine.js`):

**INCLUÍDOS (combate — no hash):** `key` (identidade), `hp` (F2.3, maxHp), `elem` (fase-mod, alvoElem/atacanteElem, variedade de tipos), `classe` (redução contra-classe), `funcao` (lida em `agir`, l.806), `faccao` (faccaoConta/curadorFaccao, l.643/726), `passiva` inteira menos `nome`/`desc`, e cada habilidade `ab[]` menos `nome`/`desc` — ou seja `slot, cost, cd, cdSe, alvo, classe/classePorModo, fx (INTEIRO), opcoes (menos o `nome` de cada opção — escolhida por ÍNDICE), alterna, modos, umaVez, ignoraInalvejavel`. Ambíguos `inicial` (flag de economia) e `provacao` (a Ordália do próprio deus) **incluídos por dúvida** (não são de tela; incluir = no pior caso re-carimba, o lado seguro).

**EXCLUÍDOS (tela — fora do hash):** `nome`+`curto` do deus; `nome`+`desc` de cada habilidade; `nome` de cada opção; `nome`+`desc` da passiva. Todos comprovadamente só-de-tela: as habilidades são buscadas por `slot`, as opções por índice, `ultHabilidade` só guarda `fx`/`alvoSpec` (não o `nome`) — o `nome`/`desc` nunca chega a uma decisão de combate.

**A DIREÇÃO DO ERRO (o ponto):** erra-se para o lado de RE-CARIMBAR. Campo novo (combate ou não) e campo ambíguo entram no hash → mudança força re-carimbo (barulho, seguro). Só um rótulo comprovadamente de tela sai. Assim um novo campo de COMBATE nunca passa em silêncio — o erro perigoso.

**MIGRAÇÃO (decisão do dono, aprovada):** mudar o escopo do hash invalidou os **100** carimbos de uma vez (o hash antigo continha `nome`). PAREI e reportei os 100 (passo 4). Aprovado o **REFRESCO DE HASH** (`tools/recarimbar_hash.js --aplicar`): recalcula SÓ o campo `verificacao.hash` com a nova função, mantendo `nivelIA/veredito/lancesNesteCaminho/comDica/nos/ms/caminho` intactos. NÃO re-resolve — o dado de combate não mudou, só o escopo do hash, então o veredito medido segue válido; a migração é incapaz de alterar um balanço em silêncio (o solucionador não roda). Os 100 arquivos mudaram só a linha do hash.

**GUARDA (babá, `solucionador.test.js` §263, seção 5b), provada nas DUAS direções:** (A) cosmético — `nome`/`curto` do deus, `nome`/`desc` da habilidade, `nome` da opção, `nome`/`desc` da passiva — NÃO invalida; (B) combate — um custo, uma recarga (cd), um número de dano (fx.v) — invalida; (C) nuance: `fx.nome` (chave de contador/dot) É combate → invalida, não é rótulo. Tirar qualquer asserção quebra o teste. Build limpo (0 velhos), suíte inteira verde.

## §262 — CONSERTOS DO §261, na ordem de prioridade do dono. Quatro achados; um deles era erro de medição MEU (não bug). O motivo comum: os cortes vieram da Cinzel REAL (mais larga/alta que o fallback contra o qual §211/§259 foram medidos); onde é dado, o conserto é no DADO, não no CSS.

**C1 — dica-mecânica inteira nos 6 atos-AULA (o único corte que perdia uma REGRA).** `.camp__mecdica` clampava em 2 linhas e elidia a explicação da mecânica — na tela do Prólogo que ENSINA quem nunca jogou. MEDI onde tirar espaço no painel de 290px: a dica precisa de no máx. 3 linhas (3 dos 6 atos passavam 1 linha, +12px); o `.crec` (chip de recompensa) tinha `height:36px` sobrando. **Decisão:** `-webkit-line-clamp` 2→3 e `.crec` 36→24 (cede os 12px). RECUSADO encolher a fonte da dica (era último recurso, decisão do dono) — o espaço apareceu no chip. Verificado: 6 dicas INTEIRAS (sem elisão do clamp), painel não estoura, botão CONTINUAR visível, em design 780 (mais apertado) e 926.

**C2 — "3º card de inimigo passa da borda" NÃO REPRODUZ: era erro de medição MEU no §261.** A tabela do §261 listou isso como corte. Ao consertar, medi o content-box REAL do painel com uma SONDA (`div width:100%`), não por `clientWidth` — e o `clientWidth` do §261 vinha ~4px curto, inventando um estouro fantasma de ~2-3px. Verdade: a folga à direita do último card é ≈0 em TODA a faixa de design [780..1200]; os cards encaixam simétricos ao lado VOCÊ, a 15px da borda dourada. **Nada a consertar no layout.** O que de fato passava da borda na captura era o TEXTO do nome do inimigo — que é o C3. (Divergência registrada como o §259→§260: reportei o erro em vez de "consertar" o que não estava quebrado.)

**C3 — NOME CURTO no DADO do bestiário (o corte sistemático).** A caixa do briefing (`.cinim__nome`, ~51px em design 780) cortava 7 de 12 criaturas; a do retrato de batalha (`.portrait__nome`, ~92px) cortava as de nome longo. RECUSADO encolher a fonte e RECUSADO alargar a caixa — em vez disso, campo `curto` no DADO (`data/bestiario/*.json`), usado SÓ nas duas caixas apertadas (briefing + nome sob o retrato); o nome inteiro fica no `title` e em todo o resto (rodapé do kit, leitura, cabeçalho). Os 12 curtos = o substantivo (epíteto fora): Náiade · Vidente · Guardião · Elemental · Ceifador · Servo · Ghoul · Silfo · Quimera · Autômato · Golem · Aparição. Medidos com a Cinzel real ANTES de aplicar: TODOS cabem nas duas caixas (o mais largo, "Elemental", 45<51 e 64<92). DEUSES excluídos (só "Mula sem Cabeça" corta, 1 em 100 — ellipsis aceitável; não se inventa curto para os 100 deuses). `metaComb` SUBIU para `src/ui/base.js` (o briefing e o campo de batalha usam; ui→ui é proibido pela build). `curto` entrou em `CHAVES_DEUS` (validador). **bragi re-carimbado** — a Provação dele usa vidente_corrompido/golem_runico, então o `curto` mudou o hash da cadeia; `curto` não muda combate, a solução é a mesma, só o carimbo.

**C4 — apelido sem corte vertical (2px, mas é a barra de identidade, na tela a partida inteira).** `.prof__nick` (Cinzel 800 11px) cortava ~2px em cima/embaixo: a Cinzel (maiúsculas altas + acentos Á/Ã/Ç) precisa de ~13px de caixa, e `line-height:1` dá 11px — o `overflow:hidden` do ellipsis horizontal cortava a tinta que sobra. **Decisão:** `line-height:1→1.3`. A barra de 46px tem folga de sobra. Verificado: apelido de 16ch acentuado sem corte-V em DPR 1/2/3 (ch14/sh14, antes ch11/sh13).

**A LACUNA DO PvP — FECHADA (positivo).** O §261 não mediu o lobby porque exigia servidor. Subi o servidor real no sandbox (`npm run serve` / localhost:8788) — o fallback "O PvP precisa do servidor" SUMIU. Medido o lobby com apelido de 16ch pior caso (larguras máximas + acentos) em design 780: `#pvpnick` (input), `.pvpnick--fixo` (apelido EXIBIDO inteiro), botões ENTRAR NA FILA / RANQUEADO, estado "Na fila…" — **nenhum corta**, 0 erros de console. O in-battle `.prof__nick` a 16ch já é coberto pelo C4. **Banner de fim de ranque:** `ranqueadoResultado` é computado no servidor e plumbado no `MP` (partida_cliente.js), mas NÃO tem elemento renderizado em `src/ui/` (grep vazio) — não há texto na tela a medir; é UI futura, não um bug de encaixe.

**NÃO CONSERTADO (por desenho, confirmado no §261):** rótulo da trilha em 2 linhas e ellipsis da epígrafe (teaser); os botões sinalizados por `scrollWidth>clientWidth` são avanço de letter-spacing, contidos.

**GUARDA (babá, `moldura.test.js` §262, atrás do portão de fontes reais, design 780):** C1 as 6 dicas não elidem; C2 sonda do content-box — o 3º card fica dentro (folga ≥ -0.5); C3 os 12 curtos do DADO cabem nas duas caixas medidas com a fonte real E o render usa o curto (1 palavra) E todo bestiário tem `curto`; C4 apelido de 16ch sem corte-V. **Suíte inteira verde.**
...
**A PROVA de que o problema era real — e que o §259 caiu nele.** No ambiente de captura/medição, `fonts.gstatic.com` é INALCANÇÁVEL (é uma "rede alheia" §206). Então o §259 mediu e capturou o **fallback serif** o tempo todo, não a Cinzel. A prova visual: a Cinzel é um tipo **só-maiúsculas** (inscricional, sem minúsculas — minúscula vira capitular menor); as capturas do §259 mostram os nomes em **caixa mista** ("Alianças, conflitos…"), que só o serif (com minúsculas) produz. Medido: a string "Alianças, conflitos e oportunidades" dá **serif 387px** (1 linha) × **Cinzel 537px** (2 linhas). O §259 reportou ~335–387/1 linha = o SERIF. **A Cinzel real, que o §260 agora carrega em qualquer rede, é mais larga.**

**PESOS EMBARCADOS — só os que o jogo USA (varredura CSS+JS).** O `<link>` pedia Cinzel 500/700/900 e Rajdhani 500/600/700 — e é EXATAMENTE o conjunto a embarcar. O CSS declara pesos FORA disso (Cinzel 400-bare/600/800; Rajdhani 400-body/800), mas o Google só serve {500,700,900}/{500,600,700}, então esses pesos **encaixam no vizinho mais próximo** (Cinzel 600→700, 800→900, 400→500; Rajdhani 800→700, 400→500). Embarcar EXATAMENTE os 6 do Google preserva esse encaixe — embarcar os pesos declarados a mais MUDARIA o visual (600/800 renderizariam de verdade). Cinzel é **variável** (mesmo woff2 p/ 500/700/900), PINADA a cada peso p/ manter o encaixe.

**SUBSET latin + latin-ext, woff2 — os arquivos EXATOS do Google.** Baixei os woff2 que o `css2` serve (Cinzel v26, Rajdhani v16), subset latin (cobre o português: U+0000-00FF tem os acentos) + latin-ext (rede de segurança; nunca chega a carregar). Cinzel: 2 arquivos (variável, reusado) = 40 KB. Rajdhani: 6 (3 pesos × 2 subsets) = 82 KB. **Total 8 arquivos, ~122 KB**, em `web/fonts/`, copiados p/ `dist/fonts/` pela build (o mesmo laço recursivo das artes). **Usar os bytes EXATOS do Google → métrica idêntica → zero diferença de layout entre "hospedado" e "do Google"** (o risco do item 5 não existe: é o mesmo arquivo).

**`font-display: block` (justificado, medido).** Com a fonte local o decode é sub-frame (woff2 de 13–26 KB, sem rede). `block` NUNCA pinta o fallback (sem o flash de métrica errada do `swap`, que era o defeito) e NUNCA cai p/ fallback a sessão toda (o risco do `optional` em aparelho lento — justo o §206). No teste sem rede, as fontes resolvem imediatamente (`fonts.check` true logo após a carga), então a janela invisível do `block` é imperceptível. `swap` e `optional` reintroduziriam, cada um à sua maneira, o problema que estamos removendo.

**REMOÇÃO das 3 linhas do Google** (2 preconnect + o stylesheet). No dist publicado NÃO sobra requisição a terceiro: 0 `googleapis`/`gstatic`, 0 `<link>` externo. (O `http://www.w3.org/2000/svg` é NAMESPACE de SVG, não busca; o `192.168.x.x:8788` é exemplo de IP LOCAL em texto de ajuda, não requisição.)

**MEDIÇÃO SEM REDE (o ponto de tudo isto) — bloqueei todo http(s) e medi no dist, viewport do dono:** **0 requisições** na carga; Cinzel e Rajdhani resolvem (`fonts.check` true); a arte é DE FATO Cinzel (537px ≠ serif 387px). **A verdade dos nomes, agora contra a Cinzel real:** dos 13, **11 cabem em 1 linha**; **2 QUEBRAM em 2 linhas** — "A Ruptura entre Céu e Terra" (414px) e "Alianças, conflitos e oportunidades" (537px), os dois que o dono marcou como longos. **Os 13 satisfazem o critério do §259 para quebra:** a legenda começa abaixo do cabeçalho de 51px (menor topo 80), o texto da história cabe, e **nenhum CORTA** (quebram limpo, não estouram). Não é o caso "CORTA = defeito". **Divergência do §259 REPORTADA, não consertada:** o "1 linha" do §259 era o serif; a escolha entre aceitar as 2 linhas, diminuir a fonte ou encurtar os 2 nomes é do dono (§259). No aparelho que carrega Cinzel, o §260 NÃO muda nada — só torna a Cinzel confiável em qualquer rede.

**GUARDAS (`moldura.test.js` §260, babá provado):** (1) nenhuma referência a Google/domínio externo no dist (re-adicionei o link → 3 falhas); (2) os 8 woff2 no `dist/fonts/` + o `@font-face` aponta p/ eles (apaguei um woff2 → falha); (3) SEM REDE, 0 requisições e as famílias resolvem em Cinzel/Rajdhani, NÃO no fallback (a arte cai no serif → Cinzel 387 == serif 387 → falha); + os 13 nomes sem corte, legenda>51, texto cabe. **Verificado no DIST com a rede BLOQUEADA** (ato prologo-01 e uma batalha): tipografia correta, 0 requisições. **Suíte inteira verde.**

## §259 — CAMPANHA: as 13 artes de ato chegaram. A campanha sai do placeholder (§213) e passa a ter ARTE PRÓPRIA por ato, gerada dos briefs de `docs/campanha-prologo-cap1.md`.

**A ENTREGA.** 13 webp **1200×900** (paisagem 4:3, o formato do slot medido no §254/README), nomeadas EXATAMENTE pelas chaves `arte` que a build espera (`prologo-01..07`, `cap1-01..06`), **~2,47 MB no total**. Copiadas para `web/banners/campanha/` sem converter/redimensionar/renomear. A checagem `_arteOk` da build (§213) acende **os 13** — todos saem do placeholder e passam a emitir `<img>`; **nenhuma 404** (tabela ato→arquivo→_arteOk toda ✓). Os dois `-fundo` de capítulo (`prologo-fundo`, `cap1-fundo`) seguem opcionais e ausentes — não fazem parte desta entrega e nenhum ato depende deles (todos têm arte própria agora).

**MEDIÇÃO NOVA — a LARGURA do nome (o §253 mediu só a ALTURA da legenda).** Na coluna de 461px, a legenda tem ~**398px** (461 − 38 esq − 24 dir). Medido no dist (viewport do dono, **com Cinzel carregado** — `document.fonts.ready`, senão o fallback serif falseia a métrica): **os 13 nomes cabem em UMA linha, nenhum corta, nenhum quebra.** O mais longo, "Alianças, conflitos e oportunidades" (35 ch), renderiza a **~335px** em Cinzel 900 26px — 63px de folga. "A Ruptura entre Céu e Terra" (27 ch) idem. A legenda começa sempre ABAIXO do cabeçalho de 51px (o menor topo medido foi 64px) e o texto da história não é empurrado para fora. **Nenhum defeito de corte** — nada a decidir sobre fonte/duas-linhas/encurtar.

**PESO.** O HTML do dist fica em **1,93 MB** (praticamente inalterado — só os 13 booleanos `_arteOk` viraram `true`; **nada embutido**: o `<img src="banners/campanha/<arte>.webp">` é montado em tempo de render, 0 blobs webp gigantes no HTML). As 13 artes são **assets separados** em `dist/banners/campanha/` (2,6 MB), carregados **um por ato** sob demanda. Crescimento líquido do dist ≈ +2,47 MB de assets, fora do HTML.

**VERIFICADO NO DIST** (viewport do dono 800×360, DPR3), quatro atos dos casos difíceis: **prologo-01** (composição de metade superior, melhor caso — texto legível sobre céu escuro), **prologo-06** (nome longo — cabe), **cap1-03** (nome mais longo, 35 ch — cabe em uma linha), **cap1-06** (assunto no terço direito: Hel e Fenrir ficam sob o véu de 90° e **perdem brilho, mas seguem reconhecíveis** — observação de composição, não defeito; o §253 pôs o véu de propósito e o README avisa "não ponha o assunto nas bordas"). Em todas, o texto da história continua legível sobre a arte e o nome não invade o cabeçalho.

**GUARDA (do lado CHEIO agora).** A GUARDA 5 do §252 (arte ausente → placeholder, nunca `<img>` 404) segue verde. Acrescentei a **GUARDA 5b**: os 13 atos do Prólogo + Cap 1 têm arte PRESENTE — cada um com `_arteOk` aceso, arquivo no disco e `<img>` na tela (não placeholder). **Babá provado:** renomeei `cap1-05-o-que-nezha-viu.webp` e o teste quebrou (o ato caiu para placeholder, `_arteOk=false`); restaurado, verde. **Suíte inteira verde.**

## §258 — COMBATE: restaurar a hierarquia "retrato > ficha" do §214, crescendo o RETRATO (não encolhendo a ficha). Fecha a ressalva reportada no §257.

**O CONTEXTO.** No §257 a ficha virou círculo e cresceu para 90 (medido: a 92 o respiro até o inimigo caía a 30px e furava o §239). Isso passou a ficha (90) à frente do retrato aliado (88px de largura), **invertendo a hierarquia "retrato MAIOR que a habilidade"** que o §214 copiou do Naruto-Arena — e a asserção que a protegia virou descrição da inversão. **Decisão do dono: a ficha FICA em 90; quem cresce é o retrato.** Por quê pelo retrato: a moldura circular já resolveu o achado (mediana do medalhão 92%, 31% preenchem a borda), então encolher a ficha custaria alvo de toque sem ganho; e o retrato de 88×64 estava pequeno numa banda de fileira de 98px — sobrava altura.

**MEDIÇÃO da banda (aliado, palco 926).** A banda tem **98px**. O que a ocupava (empilhado): retrato 88×**64** + vão 2 + nome 11,5 + vão 2 + vida 13 = **92,5px** — só **5,5px de folga**, e o retrato ART era só 64 de 98. A altura estava presa pelo EMPILHAMENTO, não pela banda.

**O CONSERTO — retrato 94×94, nome e vida SOBRE a arte.** Subi o retrato para **94×94** (maior que a ficha de 90 nas DUAS dimensões → hierarquia restaurada). Para caber na banda de 98, o **nome** e a **barra de vida** deixaram de empilhar ABAIXO e passaram a SOBREPOR a arte (véu escuro), como a faixa de efeitos já fazia na base — **não é padrão novo** (a tela do ato §253 também põe nome sobre arte). Empilhamento na base, de baixo para cima: **vida** (bottom 0) · **nome** (bottom 14) · **efeitos** (bottom 32, só quando há). O TOPO fica livre para o rótulo de time "VOCÊ/CPU" (§215) — que colidia com o nome quando testei o nome no topo; movê-lo para a base resolveu sem tocar no §215.

**LEGIBILIDADE DA VIDA — não piorou, MEDIDO.** A barra é OPACA (bg `#3a1c22` + fill em cores sólidas), então o rótulo lê contra o PREENCHIMENTO, não contra a arte. Prova no dist (pior caso): forcei a arte do retrato a BRANCO puro e medi o canto da barra — **L 0,445 (verde do fill), NÃO branco** → a arte não vaza. O rótulo (branco, L 1,0) sobre o fill dá ~2,4:1 no corpo verde, com sombra preta forte carregando o contraste efetivo — **idêntico ao de antes do §258** (a barra não mudou, só de lugar). Se a sobreposição tivesse piorado o contraste, o combinado seria retrato menor > vida difícil de ler; não foi o caso.

**SIMETRIA (item 4).** Aliado e inimigo usam o MESMO `.portrait` → crescem juntos para 94×94, automaticamente. O inimigo (coluna `.brow__enemy` de 146) segue à direita; o retrato dele é do mesmo tamanho do aliado.

**RESPIRO preservado (§239).** Crescer o retrato ALIADO em largura (88→94) NÃO empurra as fichas: o retrato cresce DENTRO da coluna fixa `.brow__ally` de 102px (as fichas começam no fim da coluna, inalterado). O retrato INIMIGO cresce dentro da sua coluna de 146. A trava `respiro > vão+30` é estrutural: vão e respiro caem juntos com a largura (ambos ~/2), então a margem fica a mesma que já tinha (o `82>81` se mantém). Medido: as fichas ainda terminam em ~764, respiro ~38.

**TESTES.** `interface.test.js`: **3 asserções** — a hierarquia que o §257 reescreveu VOLTA a ser "retrato > ficha" (agora de verdade, nas duas dimensões) + a nova de SIMETRIA (aliado=inimigo). `moldura.test.js`: **+1 seção** (browser) — hierarquia medida em px reais, simetria, **vida OPACA** (arte branca por trás não vaza: canto L < 0,5) e **efeitos cabem** (6 no pior caso = FX_MAX 5 + "+N", dentro do retrato). **Nenhuma asserção afrouxada** (as do §239/respiro seguiram de pé sozinhas). **GUARDAS babá provadas:** retrato 88×64 → a hierarquia quebra; vida translúcida → o canto vai a L 0,634 (arte vaza) e o guarda quebra. **Verificado no DIST** (viewport do dono 800×360, DPR3): fileira com o retrato 94 e a ficha 90 lado a lado (retrato maior); unidade com vida baixa (18, warn) + efeitos ativos provando a vida legível sobre a arte. **Suíte inteira verde.**

**O QUE NÃO MUDOU:** a ficha em 90 (96 recolhido), o gesto TOQUE, o invariante #3, a leitura só no rodapé (§256), o painel de 262, a faixa de efeitos e o cap FX_MAX=5 com o "+N".

## §257 — COMBATE: a ARTE que não preenchia a moldura. Dois achados do dono JOGANDO (§217): o retrato do inimigo era CENA, não retrato; e a moldura quadrada mostrava o FUNDO do medalhão circular. Conserto VISUAL, escopo fechado.

**ACHADO 1 — o retrato do inimigo mal enquadrado (arte, não CSS).** O `.slot>img` já usa `object-fit:cover;object-position:center 18%` (certo para retrato de deus, rosto no alto). As 12 artes do bestiário do §254 vieram como CENAS quadradas de 1080 (o bicho no meio de um cenário); recortadas para o box 88×64 sobrava paisagem e faltava bicho. **Conserto: troca dos 12 arquivos** de `web/bestiario/` por versões recortadas no assunto (cabeça e torso), **384×384**, entregues pelo dono. **NÃO mexi no CSS** — a troca de arquivo resolve. Confirmado no dist (Quimera/Golem/Ceifador agora preenchem o retrato). Peso: 12 arquivos = **234.454 B ≈ 229 KB** (o dono estimou 161 KB; a diferença é do encode, dimensão/assunto batem — assets, não inflam o bundle).

**ACHADO 2 — a ficha era um quadrado com o disco no meio.** Cada `web/skills/skill-<deus>-<slot>.webp` é 320×320 com um MEDALHÃO CIRCULAR sobre fundo escuro; a moldura `.skill__disc` tinha `border-radius:10px`, então mostrava a sobra de fundo nos cantos — o "embaçado". **Decisão do dono: deixar só o círculo e crescer a ficha.**

**(1/5) A FICHA vira CÍRCULO, com TODOS os estados.** `.skill__disc` e as máscaras `.skill__cd/.skill__lock/.skill__na` passam a `border-radius:50%`. Removi o `outline` do disco (o outline NÃO segue border-radius em engines antigas → cantos quadrados; a separação fica pelo anel colorido do elemento + fundo escuro do tabuleiro). Sobreviveram, todos em círculo: anel por elemento, tracejado da universal (`.skill--uni`), 2px de habilidade/milagre, `is-ready` (brilho), `is-armed` (anel dourado), e os dessaturados do §238 (`nv-indispon`/`nv-recuo` — o teste de saturação amostra o miolo, indiferente à forma). O disco do RODAPÉ (§256) recebeu o mesmo: `.leitura__icon.is-skill` e os chips do kit `.kchip__art` viram círculos (a recarga do chip migra do canto p/ o topo-centro, que no canto cairia fora do círculo) — senão a mesma arte apareceria redonda em cima e quadrada embaixo. (`.foesk` que o dono citou é vestigial; o disco real do rodapé é `.leitura__icon`/`.kchip__art`.)

**(2) O ZOOM do medalhão — MEDIDO nas 401 artes, decisão de arte reportada.** Detectei a extensão do medalhão contra a cor do canto (sharp) nas 401: **diâmetro** min 68,8% · p5 78% · p25 85% · **mediana 92%** · p75–p95 100% (**31% já preenchem a borda**, 125 artes); **centro vertical** mediana 49,7% (quase todas 45–55%). E o gabarito é inconsistente **em ESPÉCIE, não só em tamanho**: a maioria é medalhão de aro metálico com margem escura (~78%), mas há artes de sangramento total (redemoinhos que vão à borda), CENAS inteiras (a forja do Ogum, sem medalhão) e MOEDAS em fundo branco (a coruja da Atena, ~55%). **Nenhum zoom único serve para "preencher":** encher os de 78% pede ~1,28× e isso CORTA os 31% que já preenchem (aro e cena). O único valor que respeita as duas travas do dono (não cortar o aro dos de 82% E sem borda nos de 100%) é **scale 1.0, object-position centro** — que é o que a cobertura quadrado-em-quadrado já faz. **Portanto NÃO amplio: a moldura circular já corta os cantos quadrados (o "embaçado" real); o preenchimento perfeito dos medalhões pequenos é RE-CORTE de arte, não CSS.** Reportado ao dono.

**(3) CRESCER a ficha: 78 → 90 (MEDIDO).** Com o painel aberto a fileira tem 675px; as 4 fichas + 3 vãos de 8 terminam, a 90, em ~764 (palco 926), deixando **~46px de respiro** até o retrato do inimigo (146) e ~4px na vertical (faixa ~98). **Escolhi 90, não os 92 que o dono citou:** a 92 o respiro cai para **30px**, abaixo do padrão de "respiro largo" do §239 (que quer respiro > vão+30 ≈ 37) — 92 apertaria o inimigo. 90 é o maior que passa essa trava (91 já derruba o respiro a 34). Recolhido (painel fechado) a ficha vai a 96 (o teto é a vertical, ~98, não a horizontal). **⚠ Efeito colateral REPORTADO (a trava do dono: "se precisar afrouxar, PARE e diga"):** a 90 a ficha fica MAIOR que o retrato aliado (88px de largura), **invertendo a hierarquia "retrato > habilidade" do §214 (Naruto-Arena)**. Como o dono pediu explicitamente ficha maior ("preencher a tela", citou 92), tratei isso como decisão dele e ATUALIZEI a asserção (de "retrato > ficha" para "retrato não encolheu 88×64 + ficha cresceu"); se preferir manter a hierarquia, a ficha teria de ficar ≤ 87 (cresce pouco) — diga e eu reverto.

**(4) OS ORBES de custo.** `.skill__cost` saiu de `bottom:-4px` (pensado para base reta) para **`bottom:5px`**: a cápsula assenta no ARCO INFERIOR do círculo, DENTRO da pegada de 90px — assim não colide com a fileira de baixo (pendurada, colidiria) e cobre só o aro de baixo, não o miolo do medalhão. Fundo opaco + anel = ligada e legível sobre qualquer arte. O "GRÁTIS" segue o mesmo selo.

**MIGRAÇÃO DE TESTE (este corte MOVE geometria).** `interface.test.js`: **~9 asserções** — a seção 4b2 deixou de afirmar "RETÂNGULO 10px" e passa a "CÍRCULO 50%" (disco + 3 máscaras), a comparação "retrato > ficha" virou "retrato não encolheu + ficha cresceu" (a inversão acima), e entrou a trava do custo no arco inferior; mais a nova seção **4b3b** (guardas §257). `moldura.test.js`: **2** — o teto absoluto do fim das fichas (730→770, geometria nova) e um comentário; a asserção de "recolhido cresce" seguiu de pé sozinha (90→96). `enquadramento`/`render_sweep`: 0. **AFROUXEI 1 asserção** (a hierarquia retrato>ficha) — reportada acima com o porquê e a alternativa, como o dono pediu. **GUARDAS babá** (provei que quebram — revertendo o disco a 10px caem 3+): sem canto quadrado em NENHUM estado (normal/recarga/travada/armada), alvo de toque ≥76 (invariante; a ficha está 90), custo/GRÁTIS no arco inferior sem esconder o medalhão, e o disco do rodapé (`.leitura__icon.is-skill`/`.kchip__art`) circular.

**O QUE NÃO MUDOU:** gesto TOQUE (sem arraste); invariante #3 (sem nível, sem vida variável, 120 p/ todos); a leitura mora só no rodapé (§256); o painel de 262 não encolheu (outro corte). **Fora de escopo, não mexido:** a arte por-habilidade dos inimigos-criatura segue em branco no disco (não há arquivo do bestiário por habilidade — mesmo estado do §219/§254). **Verificado no DIST** (viewport do dono 800×360, DPR3): fileira com 4 fichas circulares + orbes; ficha em recarga e travada (estados circulares); retrato do inimigo com a arte nova; kit no rodapé com o disco circular. **Suíte inteira verde.**

## §256 — COMBATE, corte 1: a LEITURA desce para o RODAPÉ. Arquitetura de informação, não pele. A leitura tinha dois endereços; o kit do inimigo era efêmero (achado do dono). Agora o rodapé é o endereço ÚNICO; o painel fica SÓ no histórico.

**O DEFEITO — dois endereços para a mesma coisa.** A leitura estava partida. O RODAPÉ (`.acaoestado`/`.leitura`, §238) já lia a habilidade tocada; mas o PAINEL de 262px à direita acumulava QUATRO trabalhos: histórico, **kit do inimigo** (toque longo §214), leitura de habilidade/efeito e resumo do turno. O olho não sabia para onde ir: a mesma pergunta ("o que isso faz?") era respondida ora embaixo, ora à direita. Pior, o dono, **jogando**, achou que **o kit do inimigo sequestra o painel e depois fecha** — "kit efêmero": abre por cima do histórico e, quando o painel troca de conteúdo (o oponente age, o resumo entra), some sozinho, no meio da consulta.

**O CONSERTO — um endereço só.** Três dos quatro trabalhos DESCEM para o rodapé, que passa a ser o endereço ÚNICO da leitura: (1) a leitura de habilidade/efeito já morava lá; (2) o **KIT do inimigo** abre no rodapé, não no painel — o **atalho do toque longo §214 SOBREVIVE**, só muda o destino; como o rodapé não fecha, o kit **PERSISTE** até outra leitura tomar o lugar (conserta o efêmero); (3) o **resumo do turno** também desce. O PAINEL guarda **SÓ o histórico, SEMPRE** — nunca troca de conteúdo. **Por que o painel NÃO vira sobreposição:** na resolução, o processo do oponente é **oculto** (F0.7) — o LOG é o único canal de "por que perdi 45 de vida". Informação que precisa de um toque para aparecer é informação que some; o histórico fica à vista, fixo.

**"Qual dos três inimigos", resolvido.** `peekKit` = o uid do inimigo que sofreu o toque longo. **Segurar outro** troca de inimigo; **tocar um chip** troca a habilidade DENTRO daquele inimigo. O ✕ é o fechar deliberado. O card do rodapé nomeia o inimigo (`NOME · HABILIDADE`) e marca "KIT INIMIGO · segure outro para trocar".

**O PORTÃO DE MEDIÇÃO (reportado, não decidido sozinho): o kit CABE em 86px.** Medido no dist (viewport do dono, palco 949×86), o kit do §219 reusa o card `.leitura`: a descrição rola em `.leitura__txt` (max-height 44, `overflow-y:auto`), a tira de chips vive em `.leitura__kstrip` (chips 42px) à esquerda do ENCERRAR TURNO. Todas as caixas-filhas ficam DENTRO do rodapé (kchip bottom 409 < footer bottom 427), `overflowFooter:false`. **O rodapé NÃO precisou crescer** — nada foi restilado, nenhuma geometria mudou (ficha 78×78, retrato 88×64, moldura §239, tira de efeitos, painel 262 — tudo intacto; encolher o painel é o corte 2). O gesto continua TAP; o toque longo continua sendo o único gesto de pressão.

**MIGRAÇÃO mínima (só ONDE a leitura aparece).** `interface.test.js` seção 4b re-apontada do painel para o rodapé (`.panel .kitwrap`/`.kstrip`/`.kitdet` → `.footer .leitura__kstrip`/`.leitura__nome`/`.leitura__icon`/`.leitura__txt`/`.leitura__cd`/`[data-kitclose]`): **~17 asserções tocadas**, todas de endereço (a geometria seguiu de pé — o ícone da leitura é 52px, então o "arte grande, legível" migrou sem afrouxar). `perspectiva.test.js` seção 11 (resumo): **2 asserções**. `moldura.test.js`: **0** (não fala de kit nem resumo). Bem dentro das "poucas dezenas".

**GUARDAS (`interface.test.js` 4b + a nova 4b3, babá — provei que cada uma quebra):** (A) toda leitura tocada vive no rodapé, e o painel permanece HISTÓRICO — nunca a leitura; (B) o painel é SEMPRE o histórico (ocioso, kit aberto, resumo pendente); (C) **anti-efêmero:** com o resumo do oponente pendente, o kit aberto NÃO é despejado (fica no rodapé, `peekKit` intacto); (D) o toque longo §214 abre o kit no RODAPÉ (novo destino), nunca no painel. **Prova babá:** revertendo `painelConteudoHTML` a mostrar o kit → 6 falhas; removendo o ramo do kit do rodapé → 5 falhas. **Verificado NO DIST** (viewport do dono 800×360, DPR3): (a) habilidade tocada com a leitura no rodapé + painel histórico; (b) kit do inimigo aberto no rodapé pelo toque longo + painel histórico; (c) o MESMO um turno depois (kit persiste, TURNO 2, painel histórico); (d) resumo do oponente no rodapé + painel histórico. **Suíte inteira verde.** **Observação (fora do escopo, não mexido):** as artes de habilidade dos inimigos-criatura ficam em branco no ícone — não há arquivo por-habilidade do bestiário (mesmo estado do painel §219/§254); restilizar seria sair do corte.

## §255 — CAMPANHA, o caminho da derrota: a tela do ato promete a troca do emprestado (§252), e a derrota revogava a promessa. Agora a derrota tem "Voltar ao ato" além de "Tentar de novo". Semente estável, de propósito.

**O DEFEITO.** No fim de um ato perdido, a sobreposição oferecia só "Tentar de novo" (`#cftentar`), que remonta a MESMA semente com o MESMO time (`campanhaFim=null; st=montarProvacao(campanha); ir('batalha')`). Não havia caminho de volta à tela do ato sem abandonar a campanha. Mas a tela do ato promete que o slot EMPRESTADO é trocável (§252) — e a promessa morria justo quando o jogador mais precisa dela: o **ato VI do Cap 1 tem 29,5% de vitória** medida (IA gulosa), e quem perde ali precisa **trocar o time e reler a mecânica**, não repetir o mesmo às cegas. A dica da mecânica só existe na tela do ato.

**O CONSERTO — duas ações na derrota.** (1) **"Tentar de novo"** — inalterado: remonta a **MESMA semente e o MESMO time** (`campanha.aliados` já traz a troca do jogador). **Semente estável de propósito** — o ato é um quebra-cabeça que se aprende, não se rola de novo; aleatorizar destruiria isso (§24). (2) **"Voltar ao ato"** (`#cfvoltarato` → `voltarAoAto()`) — volta à tela do ato PERDIDO com ele como o ato ATUAL (não joga para o começo do capítulo nem para a home), para o jogador trocar o emprestado, reler a mecânica e reentrar pelo CTA. A **vitória não mudou** (nada de recompensa, `jaFeito`, `concluirEncontro` ou maestria).

**ONDE MORA O ESTADO DA TROCA, e por que sobrevive.** A troca vive em `campSwap` (global de módulo, `{slotIdx: chave}`), setado no seletor. O time montado é `timeDoAto()` = `campSwap[i] || slot.deus`; `iniciarAto` assa isso em `campanha.aliados`. O `renderCampanha` só zera `campSwap` quando **muda de ato** (`campVistaAto !== ato.id`). Como `voltarAoAto()` volta ao MESMO ato e **NÃO mexe em `campSwap` nem `campVistaAto`**, a troca sobrevive: a tela reabre com o time que o jogador montou, ainda trocável. (Antes deste §, nada garantia isso porque nada trazia o jogador de volta ao ato — o estado sobrevivia, mas não havia porta.)

**O VOLTAR DO ANDROID (§240), verificado.** A sobreposição de fim de ato é dirigida por `campanhaFim`, não pelo `ov` — então o `voltarNativo()` não a via e, na rota `batalha`, abriria o confirmar-sair POR CIMA dela. Corrigi: `voltarNativo()` ganhou, ANTES do ramo da batalha, um caso que fecha a sobreposição de fim de ato **para a tela do ato** (mesmo `voltarAoAto()`, preservando a troca). Assim "a sobreposição fecha primeiro" também nesta camada, e **nunca fecha o app no meio da campanha** (o `exitApp` só na home). O gesto/botão de voltar (§243) chega igual — o evento é do sistema.

**GUARDAS (`campanha.test.js`, babá — provei que cada uma quebra):** (A) depois de uma derrota existe SEMPRE "Voltar ao ato" (§210, nenhuma tela sem saída); (B) o time montado sobrevive à volta (`campSwap` intacto; a tela mostra o time do jogador, não o emprestado padrão); (C) "Tentar de novo" remonta com a MESMA semente (`st.seed` === a semente do ato — se alguém aleatorizar, quebra). **Verificado NO DIST** (viewport do dono 800×360, DPR3): a sobreposição de derrota com as duas ações, e a tela do ato após "Voltar ao ato" mostrando o time montado (Zeus·Nezha·Hades, com o emprestado trocado). **Suíte inteira verde.**
...
**A ESCALA.** Mockup 1170×540 → palco 951×428, fator 428/540 = 0,7926. Como a altura do palco é SEMPRE 428 mas a largura FLUI (780–1200), portei os valores de **Y/altura como px fixos** (cabeçalho 51, arte encosta na trilha, briefing top 63 / altura ~290, trilha 73) e os de **X/largura como percentuais/âncoras** que batem o mockup em 951 e sobrevivem nos outros aparelhos — isso também distribui sozinho os 24px de folga (927→951) na arte e no painel, sem barra preta.

**O que se manteve fiel ao mockup:** fundo radial do palco; coluna de arte com dois véus; nome do ato em gradiente dourado com `background-clip:text`; cabeçalho com botão Início (chevron + borda dourada inset), filete vertical, título "Campanha" em gradiente; painel do capítulo com **corte diagonal** (`clip-path:polygon(41px 0,…)`); painel de briefing com elencos (2 colunas), rótulo de formação **"N VS 3"** (comunica o 3×1/3×2 do Prólogo, números reais), cartões de inimigo 73px com faixa de nome, mecânica com selo "NOVA MECÂNICA", **CTA hexagonal** (clip + pseudo-borda + seta sólida), e a trilha com **linha-guia + progresso**, losango atual azul com halo, travado dim, rótulo em 2 linhas.

**O que MENTIA no mockup, corrigido com o dado (economia.json / data/campanha):** elenco real (não Zeus/Atena/Ares vs Guardião do Vento); **recompensas só gemas e essência** — apaguei os 3 ladrilhos falsos (500 moedas, 100 EXP) e o painel "RECOMPENSA ESPECIAL / Arte Exclusiva / ZEUS"; com 1–2 ladrilhos, largura limitada (sem flex:1 esticado); "/ 06" → **"/ 07"** no Prólogo (7 atos, do índice); pips de energia decorativos NÃO desenhados.

**DESVIOS REPORTÁVEIS (medidos no dist, o dono pediu para avisar):**
1. **A legenda da arte foi alargada e as fontes baixadas.** O mockup punha o texto numa coluna de 317px, fonte 12px, nome 30px — dimensionado para o texto-placeholder CURTO. Os textos REAIS da história são 2–4× mais longos e, nessa largura/fonte, **subiam para dentro do cabeçalho** (medido: legenda chegava a topo −15px de design no pior ato). Alarguei a legenda para a largura da arte (`left:38 right:24`), baixei o **texto 12→11px** e o **nome 30→26px**, `bottom` 92→44. Resultado medido: a legenda de TODOS os 13 atos passa a começar em ≥64px de design — abaixo do cabeçalho (51px), sem colisão e **sem rolar**. O texto mais longo mede 132px e cabe.
2. **O briefing apertou 8px.** No ato de batalha que ENSINA (Prólogo), o painel de mecânica somava e o conteúdo estourava o painel em 8px. Baixei o `gap` 11→9 e o `padding` 16→15 e subi a altura 284→290 (top 63→61): todos os briefings medem 290/290, sem estouro.
3. **O ato de HISTÓRIA mantém a arte em 48,5%** (não a alarguei para 63% como cheguei a tentar): com a arte larga, o texto passava POR BAIXO do painel de briefing (que começa em 49,6%). Mantendo 48,5%, o texto fica na coluna esquerda, sem sobreposição; o briefing de história mostra ornamento + "Um trecho da história" + CTA "Continuar" (sem moldura vazia).

**LEGIBILIDADE (medida na viewport real do dono, 800×360 tela cheia → escala 0,841, DPR 3):** o menor rótulo é a unidade da recompensa, 6,3px de design → **15,9px físicos**; os rótulos de 7–8px dão 17–20px físicos. Todos acima do piso de 11px físicos do §enquadramento — **nenhum ilegível**. Os 24px de folga não precisaram virar respiro extra.

**O que NÃO se perdeu do §252** (o mockup não conhecia): slot travado (cadeado, sem troca) · emprestado (⇄, abre o seletor; jogável com o default, §210) · mecânica que some sem `ensina` · placeholder §213 (nunca `<img>` 404) · trilha como navegação (vencido/atual/travado distintos) · troca de capítulo · ato de história (sem painéis de luta, sem recompensa, CTA "Continuar") · marca de recompensa coletada · **as 5 guardas de `campanha.test.js`, todas verdes** (mantive os mesmos hooks de classe/id). Verificado NO DIST: Prólogo II (batalha 3×2 que ensina), Cap 1 I (batalha 3×3, travado + emprestado), Cap 1 II (história), e o seletor de troca aberto — texto não estoura, os 7 nós não apertam, nada vaza dos 428. **Suíte inteira verde.**

## §252 — A CAMPANHA vira HISTÓRIA FASEADA (O Trono do Uno): a lista de encontros sai, entra a tela do ATO. O ato tem TIPO; capítulo é ARQUIVO.

A campanha era uma lista de 6 encontros que ensinava as regras. Vira o **Prólogo + Capítulo 1** de *O Trono do Uno, Livro I* — 13 atos (7+6), com arte, texto e uma tela de ato caprichada. Fonte de conteúdo: `docs/campanha-prologo-cap1.md` (§ do dono, commit 91d9e7f); **nada de narrativa/elenco/arte foi inventado** — tudo veio de lá.

**As duas decisões estruturais.** (1) **O ato tem TIPO:** `batalha` abre a luta; `historia` é arte+texto+Continuar, sem luta — é o que dá volume de conteúdo sem forçar briga em cena de conselho. (2) **Capítulo = ARQUIVO** (`data/campanha/indice.json` + `00-prologo.json` + `01-queda-das-barreiras.json` → `CAMPANHAS` na build), para que **capítulo novo seja DADO e não código** (a lição do §203/§212: conteúdo em arquivo, motor estável). A build valida cada ato (falha alto): batalha → chaves no catálogo MERGED + recompensa existe; `historia` → NÃO tem recompensa; Cap 1+ → 3×3 obrigatório (o Prólogo é isento).

**O Prólogo NÃO muda (balanço medido).** Os 6 encontros de hoje viram os atos II–VII, **copiados VERBATIM** (aliados, inimigos, `montar.seed`, `comeca`, recompensa, `ensina`) — um script leu `data/campanha.json` e emitiu o dado, não redigitei. O ato I é novo (`historia`, "A Era dos Planos"). O ato VI mantém `aliados:null` (o time é do jogador — a lição da escolha). Formações do Prólogo ficam 3×1/3×2/3×3 (o autômato solitário da Sentinela *é* a aula de recarga); **o 3×3 obrigatório vale do Cap 1** (veredito A do dono na Fase 1). `data/campanha.json` **fica até a tela nova renderizar no dist** (§202) — dado que a build valida e a tela nunca mostra é dívida silenciosa.

**Slot travado × emprestado.** Os deuses que a cena nomeia vêm **travados** (a cena os fornece, com/sem posse; moldura de cadeado, sem troca). Os slots que sobram vêm com um deus **emprestado** (afordância ⇄, abre o seletor dos deuses que o jogador TEM). `montarProvacao` monta o time na abertura e a **seed é independente dos aliados** (Fase 1, item c) — trocar o emprestado NÃO move o balanço medido. Sem saída morta (§210): quem não tem deus nenhum joga com o emprestado. (PENDENTE de propósito, do doc: marcar o emprestado como "conhecido" na Coleção — feature separada, não neste corte; anotado.)

**Recompensa só onde existe (a imagem-referência mentia).** A arte de referência mostrava gemas+moedas+EXP+arte exclusiva; **só gemas e essência existem** em `economia.json` (`encontro`:120 gemas · `chefe`:400+40✦). Desenhei **duas recompensas bem, não quatro pela metade** — moedas não são prêmio de campanha, EXP e arte-exclusiva não existem como sistema. `historia` não paga nada (senão o jogador aprende a pular o texto). Só a 1ª vitória paga.

**As 4 batalhas do Cap 1 — elenco exato do doc, chaves conferidas ANTES de escrever** (todas existem: aliados thor/loki/amaterasu/zeus/ares/hades; inimigos-deus ra/bennu/khnum/bastet/fenrir/hel/curupira/iara; inimigos-besta guardiao_bosque/elemental_chama/ceifador). **Seeds ESCOLHIDAS por mim** (não estão no plano) e registradas em `montar._seedNota`; todas `comeca:0` e **vencíveis sob jogo razoável** (verificado). **Taxas de vitória MEDIDAS** (IA gulosa dos dois lados, 200 seeds — um piso, o humano joga melhor):
- Ato I (Fragmentos): seed 1 · **100% (TRIVIAL)** — Thor+Loki travados + Ogum emprestado varrem os encantados. É o ato de abertura; aceitável, mas reportado.
- Ato III (Alianças): seed 1 · **43% (justa)** — emprestados default **ogum+ganesha**. Com o default original (zeus+ogum) dava 3% (quase injogável) e com brigid dava ~100% (trivial); ogum+ganesha é o meio. Kit NÃO rebalanceado — só a escolha do emprestado, que é design legítimo.
- Ato IV (Trovão sobre o Duat): seed 1 · **90% (fácil)** — Zeus+Ares+Hades (nomeados no doc).
- Ato VI (Caminho do trovão): seed 2 · **29,5% (difícil — clímax)** — Zeus+Ares+Hades contra Fenrir+Hel+Ceifador.

**A tela do ato (palco 951×428, orçamento 48/316/64 da Fase 1).** ESQUERDA: arte na altura toda + numeral ("TRECHO I"/"ATO I") + nome em Cinzel + texto sobre base escura — **o texto NÃO rola** (o mais longo, Cap1-II, mede 169px e cabe; font 12–12,5px). DIREITA: "Você jogará com" (3 retratos emoldurados, cadeado/⇄) + "Enfrentará" (inimigos) · "Mecânica desbloqueada" (só quando o ato tem `ensina` — **some** no Cap 1) · "Recompensas" (gemas/essência, marca coletada) · **CTA em bisel** ("Continuar história" na batalha, "Continuar" na história). RODAPÉ 64px: a linha do tempo em losangos numerados (atual aceso, vencidos ✓, travados com cadeado) — é a **navegação**. CABEÇALHO 48px com painel de capítulo em corte diagonal, epígrafe, progresso (01/07) e **troca de capítulo** (setas ‹ ›, só para capítulos desbloqueados). Arte inexistente → **placeholder do §213** (a build anota `_arteOk` por arquivo presente; **nunca um `<img>` que dá 404**).

**As 13 artes que o dono vai gerar** (em `web/banners/campanha/<arquivo>.webp`, **PAISAGEM 1200×900 (4:3)** — ver a nota de correção abaixo; README no diretório): `prologo-01-era-dos-planos` · `prologo-02-incursao` · `prologo-03-criacao-do-eixo` · `prologo-04-profecia-do-uno` · `prologo-05-inicio-do-conflito` · `prologo-06-ruptura-ceu-e-terra` · `prologo-07-ciclo-dos-expulsos` · `cap1-01-fragmentos-fora-de-lugar` · `cap1-02-conselho-temporario` · `cap1-03-aliancas-conflitos-oportunidades` · `cap1-04-trovao-sobre-o-duat` · `cap1-05-o-que-nezha-viu` · `cap1-06-caminho-do-trovao`. (Opcional: `prologo-fundo`, `cap1-fundo`.)

**AS 5 GUARDAS (`tests/campanha.test.js`, babá — tire a asserção e quebra):** (1) todo ato é alcançável pela linha do tempo (§210); (2) `historia` não tem recompensa e não abre batalha; (3) batalha do Cap 1+ é 3×3 e o Prólogo é isento (dupla trava: build falha alto + teste); (4) os 6 atos do Prólogo == `data/campanha.json` de hoje (aliados/inimigos/seed/comeca) — mexeu no balanço medido, quebra; (5) arte ausente vira placeholder, nunca `<img>` 404. **Verificado NO DIST** (viewport real do dono 800×360, DPR 3): batalha (Prólogo II 3×2, Cap 1 I 3×3), história (Prólogo I, Cap 1 II), ato travado (cadeados no rodapé), e a **troca do emprestado abre** o seletor. **Suíte inteira verde.**

> **NOTA DE CORREÇÃO (§254, correção de especificação — não é decisão nova):** o formato das 13 artes de ato acima estava ERRADO — dizia "648×1008 como os banners da home". Isso é RETRATO (proporção 0,64); o slot da arte do ato é a **coluna esquerda da tela, PAISAGEM**. Medido no dist: **460×353 de design (proporção 1,304)** → **~1162×891 físicos** no aparelho do dono (800×360 em tela cheia, escala 0,841 × DPR 3). O formato de geração recomendado é **1200×900 (4:3)**; o recorte é por `cover` a partir do centro (a 1,333 numa moldura de 1,304 sobra uma fresta lateral), então o assunto não deve ficar nas bordas. A **história usa o MESMO slot** da batalha (medido idêntico, 460×353) — um só formato. `web/banners/campanha/README.md` corrigido; o número velho (648×1008) só aparecia aqui e no README.

## §251 — Fecha o fio do §248: o ancião-com-corvo da Batalha CPU NÃO é o Odin. Decisão do dono, nenhum arquivo de arte muda.

No §248 reportei uma ambiguidade: o ancião barbado da metade esquerda do banner de Batalha CPU aparece com um corvo, e o corvo é o símbolo do Odin — que é o destaque de Campanha. Como identifico deus por arte (incerto), deixei a decisão com o dono. **Decisão do dono: NÃO é o Odin.** A arte fica como está — o corvo não sai, nenhum `.webp` muda. A ambiguidade está **resolvida por decisão, não por conserto**: o ancião da CPU é outro deus (o corvo ali não o torna Odin), e o Odin segue exclusivo do cartão de Campanha. **A varredura de deus repetido entre os 8 banners (§248) está FECHADA** — nenhum deus-título repetido, sem pendências. Mudança só de registro: este §251 + a limpeza da "ambiguidade reportada" do ESTADO.md e do fecho do §248. **Nada além da suíte a rodar** (nenhum código, nenhuma arte).
...
**A VARREDURA de deus repetido (a pedido do dono) — deus em DESTAQUE de cada cartão:** campanha = **Odin** (nórdico: trono, corvos, runas) · provações = três medalhões, **Atena** (grega) ao centro + um deus-mar escamado + uma divindade solar · desafios = **Hércules** (grego) · invocação = só símbolos, nenhum deus · coleção = **Sun Wukong** (chinês) em destaque + a colagem cinza do acervo (proposital, é "seu panteão reunido") · loja = **Tanuki** (japonês) · batalha CPU = à esquerda um ancião barbado **com um corvo** tocando lira + à direita uma deusa alada egípcia · batalha PvP = **Fenrir** (nórdico) × **Kukulkán** (maia). **Nenhum deus-título se repete exatamente — a varredura está FECHADA** (o ancião-com-corvo da batalha CPU não é o Odin: decisão do dono no §251; a arte fica como está).
...
**1. O SEMANAL dá GEMA e vira "DESAFIO DA SEMANA".** Passa a creditar **Gema** (recurso de invocação — ajuda a colecionar) na 1ª vitória da semana, em vez de Essência; continua grátis e expirando por semana. Renomeado (o "Provação da Semana" confundia agora que Provação = missão). `economia.json` ganhou `semanal.recompensa.gema` (150, tunável).

**2. OS PERGAMINHOS viram DESAFIOS POR DEUS.** O jogador escolhe de quem quer o desafio, **compra com Essência (30)**, pilota o deus até vencer e ganha **maestria (+3)** dele. A **MOLDURA sai no MESTRE** (§204, limiar 30 — sistema que já existe, não se criou novo): 30 ÷ 3 = **10 desafios por moldura**. E a maestria segue vindo de vitória (§204) — **dois caminhos ao Mestre** (jogar muito, ou acelerar com desafios). No servidor do perfil: `perfil.desafios[deus] = {ativo, recargaAte}`; a vitória credita +3 ao TÍTULO (e marca o `milagre` do Mestre — o desafio desenhado É a prova de kit), **não** o `creditarMaestria` genérico (que daria +1 ao suporte).

**3. AS 9 REGRAS, cada uma com o motivo (todas implementadas + testadas):** (1) UM por deus por vez (senão estoca 10 e a recarga não significa nada). (2) VÁRIOS deuses ao mesmo tempo, sem teto (pagou; a Essência já limita). (3) NÃO expira (prazo em coisa paga vira ansiedade). (4) Perdeu a batalha? Repete de graça, fica até vencer (puzzle com solução provada — perder é aprender). (5) Pode DESISTIR, sem reembolso, e a recarga começa (senão trava naquele deus para sempre). (6) A recarga começa ao CUMPRIR ou DESISTIR, **nunca na compra** (senão sentar em cima gastaria a recarga de graça). (7) Recarga de **8h** (3/dia por deus, uma moldura em ~3 dias). (8) SÓ de deus que você TEM (você pilota o deus). (9) NÃO avança a Provação (PvP × puzzle, atividades diferentes). Custo/maestria/recarga em `economia.json.pergaminhos` (tunável).

**4. OS NÚMEROS (medidos) + a cobertura + a moldura:**
- **Maestria por desafio: 3** (confere a estimativa do dono). **Custo: 30 Essência** (confere; contra a renda de repetidos A15/S40/SS120, um frame = 300 = 20×A repetido, ou 2,5×SS; razoável, nem trivial nem absurdo). **Uma moldura:** 10 desafios × recarga 8h = **~3,3 dias** de foco (confere os "~3 dias" do dono). **As 100 molduras:** a recarga é POR DEUS e sem teto de deuses simultâneos, então em paralelo o gargalo não é a recarga (~3,3 dias) e sim a **Essência (30.000 = 100×300)** e jogar ~1.000 puzzles — um grind cosmético de fim de jogo, por desenho.
- **COBERTURA: 63 → 100/100.** O dono escolheu "gerar só os 37 que faltam". Fiz sem descartar trabalho válido: **gerei 10** (os 9 iniciais + oni, que não tinham pergaminho) com `tools/gerar_pergaminhos.js` (reusa o motor de puzzles do §203 — título fixo, suporte curado, o solucionador prova VENCÍVEL, carimbo do §148), e **promovi os 27 genéricos** (que já tinham puzzle solver-verificado, só estavam fora do acervo pela razão de aquisição do §212) ao acervo. Todos os 100 são jogáveis, sem aviso de carimbo.
- **A MOLDURA precisava existir visualmente:** havia só um SELO (estrela) de Mestre, não uma moldura. Adicionei a moldura como o cosmético mais barato (**variação de borda** em CSS, sem arte nova): `.colx--mestre` — anel duplo dourado + brilho + cantos ornamentados, distinta do "dourado = você tem".

**PROVA:** `data/economia.json` (`semanal`, `pergaminhos`), `src/perfil.js` (`desafios:{}`), `src/ui/home.js` (hub por deus + as 9 regras + o reward semanal Gema + a moldura), `src/shell.html` (moldura + linhas), `tools/gerar_pergaminhos.js`. `tests/desafios.test.js` (29 asserções: as 9 regras, os números, a moldura no Mestre, o semanal em Gema); `aquisicao`/`semanal`/`render_sweep`/`provacao_loop` migrados (cobertura 100, hub por deus). **Suíte verde.** Captura contra o dist (o hub por deus + a moldura do Mestre).
...
- **Barra de status:** plugin oficial `@capacitor/status-bar` — `setOverlaysWebView({overlay:true})` + `hide()`, chamado no boot e **reafirmado no `resume` e no `visibilitychange`** (o WebView readmite a barra ao voltar do 2º plano). Config `StatusBar.overlaysWebView` no `capacitor.config.json` para o estado inicial.
- **Barra de navegação:** pela **API de Fullscreen** que o jogo já pedia no 1º toque (§F0.6) — no WebView do Capacitor ela entra em **imersivo STICKY**, então a barra volta ao **deslizar da borda** e some sozinha. Reafirmada no resume junto com a de status.
- **Fallback nativo documentado** (`PUBLICAR-APK.md` seção 8): se algum aparelho não esconder a barra de navegação só pela tela cheia, um `MainActivity.java` com `WindowInsetsControllerCompat.hide(systemBars())` + `BEHAVIOR_SHOW_TRANSIENT_BARS_BY_SWIPE` (o imersivo oficial do Android). É a única parte que seria hand-edit — e só se precisar; o `android/` não é versionado, então isto vira uma edição local de uma vez (persiste nas builds; refaz só se recriar `android/`).

**O §240 (botão VOLTAR) segue valendo com a barra escondida — confirmado por raciocínio e teste.** O evento `backButton` é do SISTEMA: dispara pelo GESTO de voltar (ou pelo botão quando a barra reaparece), **independente de a barra estar visível**. Nada muda no tratamento (sobreposição→fecha · batalha→confirmar-sair · sub-tela→home · home→sai). `tests/plataforma.test.js` cobre o back inteiro; nenhuma mudança foi necessária ali.

**O enquadramento (F0.6b) continua certo.** Com as barras escondidas, `env(safe-area-inset-*)` encolhe; o jogo **reenquadra** (`fit()`) no boot, no `fullscreenchange`, no `resume` e no `visibilitychange` — então o palco PREENCHE o espaço novo sem cortar (o notch/recorte segue lido) nem sobrar tarja.

**Ganho de altura (medido por cálculo; o exato sai do aparelho).** ~24dp (status) + ~48dp (navegação de 3 botões) = **~72dp de altura recuperada** — ~**+18% a +21%** de espaço vertical em paisagem (navegação por GESTOS recupera ~24dp). Como o palco escala para caber, vira jogo MAIOR. O número real se lê no `chrome://inspect` (`innerHeight` antes/depois) ou no painel de diagnóstico — não há aparelho aqui para cravá-lo.

**2. A regra: reinstalar vs auto-atualizar (a lista, para o dono não perguntar).** Confirmado: **jogo atualiza pelo Render; app nativo exige reinstalar.** Atualiza SOZINHO (só publicar + reabrir): todo o jogo (HTML/JS/CSS/telas/layout/o próprio imersivo em código/o botão voltar), os dados (kits, missões/volumes/ranque, campanha, textos, balanceamento), o motor e o servidor. Exige APK NOVO: `server.url`, `appName`/`appId`, ícone, splash, adicionar/atualizar plugin ou permissão, o `MainActivity.java` da seção 8, versão de SDK/Capacitor, e a virada para o modo embutido/offline. **Cache da WebView:** raro (o `server.url` rebusca o documento a cada abertura, sem cache agressivo); se acontecer, **fechar dos recentes** resolve (força o rebusca); senão, forçar parada / limpar CACHE (não "dados", que apaga o login). Nada no código é preciso para o caso normal — só a versão de LOJA (embutida) precisaria de estratégia de cache.

**PROVA:** `@capacitor/status-bar` no `package.json`; `capacitor.config.json` (StatusBar overlay); `src/view.js` (`imersivo()` + reafirmação no resume/visibilitychange + `fit()`), guardado (no-op no navegador). `tests/plataforma.test.js` estendido: barra de status escondida no boot e **reafirmada no resume**, back independente da barra, no-op sem Capacitor. `PUBLICAR-APK.md` seções 8 (imersivo + fallback nativo + ganho) e 9 (reinstalar vs auto-atualizar + cache). **Suíte verde.** APK não gerado aqui (sem SDK/aparelho) — o run e a medição do ganho exato são do dono.
...
**1. PORTÃO DE RANQUE — e isto REVERTE o §232.** No §232 eu removi o portão de faixa achando trava dupla. O dono o quer como **ESTRUTURA QUE REVELA** (o "ser no mínimo Anbu" do Naruto-Arena), não cadeado extra — e o uso dele é melhor que o meu. **Registro a reversão com o motivo.** Reusa as **8 faixas que já existem** em `data/ranqueado.json` (não uma segunda escada competindo com o ranqueado — dá função ao ranque além do placar). Distribuição **mais generosa embaixo**, soma 91: Suplicante 18 · Devoto 15 · Iniciado 13 · Adepto 12 · Sacerdote 11 · Oráculo 9 · Herói 7 · Semideus 6. As 3 primeiras abrem **46/91 (51%)** — o casual tem ~50 deuses de caminho antes de o ranque apertar; mitiga o risco do §232 (quem perde mais que ganha fica LONGE do topo, não trancado). **Correção de nomes:** o dono citou "Hierofante/Arconte", que NÃO existem no arquivo — usei as 8 reais **por posição** (…Adepto, Sacerdote, Oráculo, Herói…); a distribuição e a rampa seguem a ORDEM, não os nomes.

**2. AS TRÊS TRAVAS CORRELACIONADAS por faixa (rampa, não parede).** Se independentes, uma missão poderia ser ranque-alto + cadeia-funda + sequência-longa ao mesmo tempo = muro. Correlacionei: a **faixa vem da PROFUNDIDADE da cadeia** (atribuição topológica — o companheiro sempre destrava em faixa ≤ a do deus, cobrado na validação), então faixa baixa = cadeia rasa, faixa alta = cadeia funda, por construção; e a **sequência sobe com a faixa**. `data/missoes.json` ganhou por missão: `faixa/faixaMin/faixaIndice`, `profundidade`, `seguidas`, `seguidasAlvo`.

**3. SEQUÊNCIAS — e o TETO veio da MEDIÇÃO (movi o número com o motivo).** Sequência é o único requisito que a DERROTA destrói — o freio honesto (mede habilidade, não tempo de cadeira). **TODA missão tem ≥1** (antes só 38 objetivos tinham). O dono pediu teto 6 (rampa 3/4/5/6). Eu **simulei** (`tools/custo_missoes.js`, jogador esperto com time misto, p=0,5, contador desde o desbloqueio) e a rampa literal deu **1808 partidas ≈ 6-8 meses** — beira o abandono que o dono mesmo definiu (as sequências são exponenciais — uma de 6 ~126 partidas a 50% — e o contador-desde-o-desbloqueio impede pré-farm, então elas quase não se sobrepõem entre elos). **Decisão do dono após ver os números: suavizar para TETO 4 (rampa 2/3/4)** → ~924 partidas ≈ **3-4 meses** (a "caçada"). Mantém as três travas.

**4. CADEIAS — a DECISÃO que as faz valer: O CONTADOR COMEÇA NO DESBLOQUEIO.** Volume e sequência contam a PARTIR do momento em que a missão destrava (companheiro possuído + ranque atingido); **vitórias anteriores NÃO contam retroativamente.** Sem isso a cadeia só esconde a missão e o jogador acumula tudo em paralelo (freio nenhum); com isso, força jogo sequencial. No servidor (`server/missoes.js`): `led.desbloqueio[deus] = {em, volBase, seqBase}` gravado quando destrava (a base fica no estado PRÉ-partida, pois o ranque da partida só é aplicado depois); `missaoCumprida` cobra `(atual − base) ≥ requisito`; um reset de sequência após o desbloqueio zera a base (a nova sequência conta inteira). **Registro explicitamente.** **A contagem de cadeias: fica em 58 (temática, intacta), NÃO 80.** O dono pediu ~80, mas chegar lá exigiria reescrever ~22 vínculos que ele escreveu à mão (§230) — degrada o tema; **decisão do dono: manter as 58 + o portão de ranque**, que deixa **73 missões "gated"** (a ordenação que ele quer) sem tocar nos motivos.

**5. A TELA agrupa por FAIXA (§234 → §241).** `renderMissoes` agora agrupa as 91 pelas 8 faixas, cada uma com quantas libera e o status ("N/N conquistados" ou **"falta subir X faixas"**); os tiles trazem o estado funcional (progresso/disponível/travada-por-companheiro/travada-por-ranque "abre em …"/conquistado). Ver o que espera nas faixas de cima faz querer subir. O contador ao vivo desconta a base do desbloqueio (§241 item 4). O detalhe do deus (§220) mostra o requisito completo (volume + sequência + "abre em [faixa]").

**OS NÚMEROS (o dono pediu; corrijo a estimativa dele).** Árvore revalidada por varredura (§202): sem ciclo · 91/91 alcançáveis · **rampa em ordem** (companheiro nunca destrava depois do deus) · caso Maia OK. **Profundidade: 8 ondas** (a Maia é a mais funda). **Custo (simulado):** a estimativa do dono (586 partidas, 59-78h) estava ~3× baixa — mesmo o ótimo do jogador para a rampa literal é ~1808 partidas; a rampa escolhida (teto 4) dá **~924 partidas ≈ 3-4 meses** a 1h/dia (6-8 min/partida). Uma consequência do §241 medida no teste: a cascata Maia agora fecha **em ESTÁGIOS** (não numa passagem) — o contador-desde-o-desbloqueio impede pré-farmar o volume Maia antes de o itzamná existir.

**PROVA:** `tools/gerar_missoes.js` (portão + rampa + profundidade + validação da ordem), `tools/custo_missoes.js` (a simulação de custo, com cenários), `server/missoes.js` + `server/contas.js` (contador desde o desbloqueio, portão de ranque, sequência por panteão), `src/ui/home.js` (tela por faixa), `tools/build.js` (slim carrega faixa/seguidas). `tests/missoes.test.js` (55 asserções: portão, desde-o-desbloqueio, cascata em estágios) e `tests/missoes_tela.test.js` (24: 8 faixas, "falta subir", travada por ranque). **Suíte verde.**

## §240 — O APK (Android), modelo SERVIDOR-APONTADO (decisão do dono). O app aponta para o Render (não embute o jogo): publicar atualiza o app sem reinstalar. appId/appName permanentes, endereço em UM lugar, splash cobre o servidor dormindo, e o botão VOLTAR do Android resolvido.

O servidor está no ar (PvP testado em dois celulares), então o APK tem para onde apontar. **Decisão arquitetural do dono: o APK APONTA para o servidor, não embute o jogo** — assim que se publica no Render, o app já mostra a versão nova, sem reinstalar ("instala uma vez, ajusta cem vezes"). A versão embutida (offline) é a de LOJA, no fim; não é agora. Isso **inverte o modelo do spike §218** (que era embutido).

**Identificador PERMANENTE (não muda depois de publicar na loja):** `appId = com.gsneves.incursionx3battle`, `appName = INCURSION` (o spike usava `com.incursion.x3battle` / `INCURSION x3`, descartados).

**1. O modelo servidor-apontado = `server.url` no `capacitor.config.json`.** O Capacitor carrega o jogo dessa URL e **injeta a ponte nativa NAQUELA origem** — então o PRÓPRIO código do jogo (servido pelo Render) fala com os plugins nativos, sem editar o projeto Android à mão. **O que muda vs o spike (tabela completa no `PUBLICAR-APK.md`):** o jogo vem do servidor (não dos ~6,6 MB embutidos) → APK bem menor; atualizar = publicar no Render (não reinstalar); precisa de internet (não offline). **`tools/cap-www.js` reescrito:** o `www/` deixou de empacotar o jogo+assets; virou um `index.html` mínimo de segurança (tela escura "conectando", em vez de branco). O Capacitor exige um `webDir`, mas com `server.url` ele NÃO é a tela que abre.

**2. O endereço num LUGAR ÚNICO: `capacitor.config.json` → `server.url`.** Hoje `https://incursion-servidor.onrender.com` (o nome do serviço no `render.yaml` do §237). Trocar de hospedagem = trocar UMA linha + `npm run cap:sync`. **De brinde, o §237 fecha o cliente:** como o jogo vem do servidor, `criarTransporteWS` usa `location.host` e o WebSocket vai para a MESMA origem — `wss://` automático em produção, zero config no cliente (a classe de bug §209/§236, já coberta por `wss.test.js`). **Servidor dormindo (Render grátis dorme em 15 min, acorda em ~30s):** um **splash nativo** (`@capacitor/splash-screen`, fundo escuro + rodinha, `launchAutoHide` com teto de 30s de segurança) cobre a espera — **não um branco de "app quebrado"**; o jogo chama `SplashScreen.hide()` assim que a UI aparece (caso comum some em ~2-3s).

**3. O botão VOLTAR do Android — RESOLVIDO (o dono pediu explícito).** O router é uma **pilha JS, sem `window.history`** — então o back do Android não teria o que percorrer e **fecharia o app de cara** (grave no meio de uma partida). Consertado via `@capacitor/app` `backButton` (funciona porque a ponte é injetada na origem do `server.url`): o back faz o que o **"‹ Início"** faz — sobreposição/menu aberto fecha primeiro; **na batalha abre o confirmar-sair (`ov='sair'`)**, não abandona sozinho, **nunca fecha o app**; sub-tela vai para a home; **só na home** (a raiz) o back sai do app (padrão do Android). Tudo em `src/view.js` (`ligarPlataformaNativa`/`voltarNativo`), guardado por `window.Capacitor` — **no navegador comum é no-op** (o jogo web segue idêntico).

**4. Ícones: os que existem SERVEM (sem arte nova).** `web/icon-512.png` (512) + `icon-192.png` já bastam de ORIGEM; o Android quer várias densidades (mdpi 48 → xxxhdpi 192) + o 512 da loja, **gerados do 512** por `npm run cap:assets` (`@capacitor/assets`). O 512 cobre tudo (o maior de launcher é 192 < 512). Ressalva honesta: para a LOJA o ideal de origem é 1024 (margem p/ o recorte adaptativo) — não bloqueia o teste.

**O QUE O APK VAI REVELAR (anotado antes — é o valor de fazer isto), no `PUBLICAR-APK.md`:** a WebView morta em segundo plano vs a reconexão da F5.4 (nunca testada em app real — trocar de app e voltar é o teste); o toque longo §214 na WebView (a lupa do sistema, fechada por `-webkit-touch-callout:none`); o desempenho da IA no Difícil num aparelho; e o back do Android (resolvido, confirmar no device).

**O LIMITE (declarado):** iPhone fica fora (Android só); sem internet o app não roda (offline é da loja); servidor fora de verdade (não só dormindo) = tela de conectando (feche e reabra). **Para a loja falta** (só listado, não feito): assinatura (keystore/AAB), conta Play Developer (US$25), política de privacidade (URL), classificação indicativa, ficha da loja, e a versão OFFLINE embutida.

**PROVA:** `tests/plataforma.test.js` (jsdom com `window.Capacitor` FALSO injetado por `beforeParse`, como a WebView faz): o splash é escondido no boot, o `backButton` é registrado, e o back faz sub-tela→home / batalha→confirmar-sair / sobreposição→fecha / home→sai-do-app, **nunca fechando no meio da partida**; e sem Capacitor tudo é no-op. **43 suítes verdes.** `capacitor.config.json` (server.url + splash), `tools/cap-www.js` (www mínimo), `package.json` (`@capacitor/app` + `@capacitor/splash-screen` + `@capacitor/assets`, scripts `cap:assets`/`test:plataforma`), `PUBLICAR-APK.md` (roteiro Mac à prova de erro) e o banner de "superado" no `CAPACITOR-SPIKE.md`. **APK não gerado aqui** (sem SDK/aparelho no ambiente): o run é do dono, sem adivinhar passo.

## §239 — FASE 4 / BATALHA, onda VISUAL (§217): a moldura que UNE o deus e o que ele pode fazer, as habilidades COLADAS no retrato, e a ênfase de turno que INVERTE sem mover nada. Nos quatro modos (a mesma tela).

Onda visual dos achados de batalha no celular. Vale para TODAS as telas de batalha — Provação, Campanha, CPU e PvP usam a MESMA `renderBatalha`. A funcional (§238) deu o "consigo LER"; esta dá o "é bonito e a leitura tem hierarquia". Num ponto eu **recomendei diferente do dono e ele acatou o motivo** (item 4).

**5. AS HABILIDADES COLAM NO RETRATO, COM UMA MOLDURA ATRÁS.** O dono mediu a referência (Naruto-Arena): as habilidades ocupam ~14%–63% da largura, coladas no retrato à esquerda — NÃO centralizadas. O nosso layout centralizava os tiles no vão (`.brow__tiles{justify-content:center}` num flex que crescia) — parecia peças soltas. Tirei a centralização: agora o retrato + os 4 tiles vivem numa **MOLDURA** (`.brow__unit`, `flex:0 1 auto` colada à esquerda; o inimigo fica FORA dela, à direita — §214 o pôs lá para o polegar, não mexi). A moldura é uma **placa emoldurada** (`.brow__unit::before`) que passa POR BAIXO do retrato e se estende atrás das 4 habilidades, unindo-os — a fileira lê como "este deus e o que ele pode fazer". **Parece profundidade, não retângulo chapado:** sombra projetada (`0 5px 14px`), **borda dupla** (borda + anel interno `inset 0 0 0 1px`), leve gradiente de textura. E a placa é **INSET no vertical** (o `::before` recua 9px em cima/baixo), então mais baixa que o retrato — o **retrato SOBREPÕE a borda de cima** e fica por cima dela (medido: o retrato "pop" 9px acima da placa).

**Geometria medida (`tests/moldura.test.js`, navegador real 926×428):** painel 0–262 · retrato **285–373** (88px) · gap 7 · as 4 habilidades terminam em **716** (o dono calculou ~710) · retrato inimigo **802–890** (o §214, imóvel) · **respiro 86px** entre as habilidades e o inimigo. *O respiro (86) saiu maior que os 62 que o dono estimou* — porque o retrato inimigo está centralizado na sua faixa (802), não colado em 772. É respiro a mais, não a menos: não corrigi, e ver a recomendação abaixo.

**4. O TURNO DO OPONENTE — RECOMENDEI e o dono acatou: a ênfase MUDA, a posição NÃO.** O dono cogitou recolher as habilidades para perto do retrato no turno do oponente. **Recomendei NÃO mover nada:** o turno alterna duas vezes por rodada; layout que se desloca a cada troca embaralha a leitura e cansa. Em vez disso, **a ênfase segue a LUZ** (iluminar o lado dele é mais claro que apagar o meu, e não move nada): o `baselayer` ganha a classe `turno-eu` / `turno-eles`; no turno do oponente a **minha moldura apaga** (borda `#40395c`) e o **retrato DELE acende** (aro rosa + halo). Metade já existia (§238 recua as minhas habilidades quando não é a vez); faltava o INVERSO — o lado dele ganhar ênfase. **Sinal INEQUÍVOCO de vez, não só texto:** a placa do lado ativo PULSA (`vezpulse`/`vezpulseFoe`) — antes o dono só descobria de quem era a vez lendo "Vez de Oponente — aguarde".

**GUARDA PERMANENTE: a posição das habilidades NÃO muda com o turno.** `tests/moldura.test.js` mede a fileira na minha vez e na do oponente e cobra que `porL, t0L, tLastR, unitL, unitR, enemyL, enemyR` sejam IDÊNTICOS (< 0.6px) — só a classe de ênfase muda. Se alguém deslocar o layout por turno, o teste pega.

**CENÁRIO / o vão à direita — recomendação (o dono pediu, sem inventar arte):** a referência tem ILUSTRAÇÃO de fundo; o nosso é gradiente escuro. **NÃO inventei arte.** O que recomendo, na ordem: (a) o respiro de 86px à direita é *bom* — é o descanso visual que separa "meu lado / lado dele"; não encher de tiles nem esticar a moldura até o inimigo (colar tudo volta a virar "peças soltas"). (b) Quando houver arte de cenário, ela entra como camada ATRÁS de tudo (`.stage__bg`, que já existe como gradiente) — trocar o gradiente por ilustração por MITOLOGIA do deus ativo é o passo natural e NÃO mexe no layout. (c) Um toque barato antes de arte: uma vinheta/textura sutil no `.stage__bg` dá profundidade sem pedir ilustração. Deixo como recomendação, não implemento agora — arte é decisão do dono.

**QUATRO MODOS VERIFICADOS (§202).** `tests/render_sweep.test.js` (seção 8): a moldura `.brow__unit` existe e contém o retrato E as habilidades (inimigo fora), com a classe `turno-eu`, em sandbox, Provação e Campanha; e no turno do oponente (vs CPU) o baselayer marca `turno-eles` e as 12 habilidades recuam. `tests/perspectiva.test.js` e `interface.test.js` seguem verdes (seletores por descendência — o wrapper `.brow__unit` é transparente).

**PROVA:** 42 suítes verdes. Captura contra o dist a 926×428: **na minha vez** (moldura acesa em ouro, retrato por cima, habilidades coladas) e **na vez do oponente** (minha moldura apagada + habilidades recuadas, os retratos dele acesos em rosa). **Guarda permanente cravado:** posição idêntica entre turnos.
...
**1. UMA AÇÃO POR VEZ, VALIDADA — o cliente NUNCA declara resultado.** O cliente pede (`jogar`), o servidor valida contra o estado autoritativo (é o seu turno? é a sua unidade? o motor aceita?), aplica e devolve. Ação inválida = `recusado` com código/motivo (`nao_e_seu_turno`, `unidade_invalida`, `acao_invalida`), nunca aplicada "na dúvida". Vitória, derrota e fim de turno são do servidor — o cliente lê `fim` do snapshot, não o inventa.

**2. INTERFACE OTIMISTA que a pureza permite (§221).** O cliente aplica a ação localmente e desenha na HORA; o servidor confirma. Como o motor é determinístico, a confirmação bate sempre (medido: partida completa, **0 divergências reais**). No `encerrar`, o cliente prevê até a **jogada inteira da IA do oponente** (mesmo motor, mesma IA) e desenha — o servidor confirma. **E SE DIVERGIR?** (não deveria): o cliente **se corrige pelo estado do servidor** (a verdade) e **grita no log** — `⚠ divergência corrigida pelo servidor (cliente XXXX ≠ servidor YYYY)`. Divergência SILENCIOSA é o pior bug de um jogo com ranque; aqui ela é sempre visível. Ação recusada pelo servidor é **desfeita** (o otimista rola de volta) e o motivo aparece.
   - **Onde os avisos moram (achado):** o `st.log` é ESTADO autoritativo e entra no hash. Anotação local do cliente (divergência/recusa/tempo) NÃO pode ir no `st.log` — poluiria o hash e faria TODO passo seguinte divergir do servidor (divergência que se PROPAGA). Por isso os avisos vivem num canal separado (`MP.avisos`); só o servidor escreve no `st.log`. A UI desenha os dois juntos.
   - **`agir` é TRANSACIONAL agora:** uma ação recusada nunca pode corromper o estado autoritativo. O servidor (e o cliente) fotografam antes de `E.agir` e restauram se o motor recusar. Rede de segurança para o invariante "o servidor é limpo".

**3. O RELÓGIO DO TURNO é do SERVIDOR.** Saiu do cliente (a barra de 0:57 rodava local). O servidor arma um disparo no fim do turno do humano (`setTimeout`) e, ao estourar, **empurra** (`push:true`) o novo estado sem o cliente pedir — o cliente desenha o que sobra do `deadline` autoritativo, sem encerrar sozinho. **Regra de esgotamento (definida — a prosa do ranqueado não tinha uma; reusa-se esta):** tempo esgotado = **o turno PASSA** (auto-passa), NÃO morte instantânea — um turno perdido já é penalidade tática real. **3 turnos consecutivos SEM nenhuma ação = derrota por ABANDONO** (anti-idle do ranqueado). Turno em que o humano AGIU não conta como perdido (justo com quem só demorou). `limiteMs` é do servidor (60s), clampado [200ms,120s] e não confiável ao cliente (só afina/testa).

**4. O OPONENTE é a IA rodando no SERVIDOR.** Testa o protocolo INTEIRO (validação, otimista, relógio, fim) sem depender do pareamento (F5.3). O `encerrar` do humano faz o servidor rodar a IA do oponente e devolver as ações dela (`cpuOps`) — o cliente desenha o turno do oponente. `montar/agir/fim` crus da F5.0 continuam como ARNÊS de determinismo (aplicam a op exata, sem dirigir a IA); a partida de verdade usa `novaPartida/jogar/encerrar`.

**5. FIM POR VARREDURA — rede de segurança (achado real, corrigido).** O motor declara vitória quando um lado é varrido, MAS só nos pontos onde roda `checarFim` (dentro de `agir`/`matar`/início de turno). Um lado varrido por efeito de INÍCIO de turno para o qual o controle VOLTA (fica ocioso) pode não passar por esses pontos, e a partida travaria (medido: humano morto, partida rodando sem vencedor até o teto de 40). O servidor, dono do fim, aplica a MESMA regra do motor como rede de segurança (`_garantirFim`: todos `!vivo` e sem renascer pendente = o outro venceu); o cliente espelha na previsão para o hash ficar em lockstep. Não toquei o motor (não exporto `checarFim`); a rede vive na borda de partida.

**6. AUTENTICAÇÃO em TODA mensagem de jogo (achado de integração).** `novaPartida/jogar/encerrar` levam o token (§222). Um bug pego na captura: o modo online do `turno.js` não repassava o token em `jogar/encerrar` → o servidor recusava toda ação com `sem_token` e a partida travava (o servidor limpo recusando, exatamente como deve). Corrigido: o token entra no `entrarModoOnline` e viaja em cada pedido. A captura de ponta a ponta é o teste de integração desta camada.

**O QUE A F5.4 (RECONEXÃO) VAI PRECISAR — anotado, com o que dá para verificar aqui:**
   - **Partida keyed por CONTA, não por conexão.** Hoje `P` vive na conexão (fecha → some, e o relógio é limpo no `close`). A reconexão exige a partida guardada por conta/id, sobrevivendo a uma conexão caída, com o token reabrindo-a. O reenvio é o **estado inteiro** (~11 KB, medido na F5.0) — sem delta, sem replay (determinismo + tudo público).
   - **No WebView, reconectar é o caminho NORMAL, não exceção (§218).** O Android MATA a WebView em segundo plano e o WKWebView do iOS suspende: toda troca de app derruba o socket. Então a F5.4 tem de tratar "voltou ao app" como rotina: reabrir o socket, reautenticar pelo token, pedir o estado atual da partida. WebSocket em WebView ainda tem política PRÓPRIA de origem/certificado (`wss://`) a confirmar no aparelho.
   - **O relógio corre na ausência.** Desconectar no seu turno gasta o seu tempo (justo: seu relógio é seu, olhando ou não) — a regra de abandono da F5.2 já é, de fato, a regra de forfeit por queda. Uma janela de graça curta antes de o turno passar é a única decisão fina em aberto.
   - **O que dá para verificar AQUI (Node):** o PROTOCOLO de reconexão — duas conexões com o mesmo token, a partida guardada por conta, o estado devolvido na volta, o relógio que correu no intervalo. É testável sem aparelho e vai para a suíte da F5.4.
   - **O que DEPENDE do seu aparelho:** o comportamento real do WebView em segundo plano (timing da queda no Android, suspensão do iOS), e a política de origem/certificado do `wss://`. Este ambiente não roda Android/iOS — é o checklist do spike §218 (`CAPACITOR-SPIKE.md`), no seu device.

**PROVA:** `tests/partida.test.js` — (1) o controlador (ação validada, servidor dirige a IA e é dono do fim, relógio auto-passa, abandono aos 3); (2) o cliente (hash byte-idêntico ao servidor, otimista bate, divergência forçada corrigida e VISÍVEL, recusa desfeita); (3) integração WS (partida COMPLETA contra o servidor, 0 divergências, fim declarado pelo servidor, o relógio do servidor empurra o estouro). **31 suítes verdes.** Captura de ponta a ponta: partida no servidor (turno 1), em curso, e o fim ("OPONENTE VENCE · encerrou no turno 8") desenhado pelo cliente.

**ESCOPO:** nada de pareamento, dois humanos, ranque de verdade ou reconexão aqui. O oponente é a IA do servidor; a F5.3 traz o PvP e a F5.4 a reconexão.

## §222 — FASE 5 / F5.1: CONTAS. Conta anônima sem login, faixa de idade guardada, exclusão de verdade, token no aparelho, protocolo agora autenticado. As decisões do §221 (a,b,c,d) entram em prática.

**O INVARIANTE DA FASE segue (§221):** o servidor é autoritativo; o cliente desenha. A F5.0 provou a fundação (44 passos, hash idêntico). A F5.1 põe a conta em cima dela.

**1. CONTA ANÔNIMA na 1ª abertura — o jogador NUNCA vê login.** Não é "conta de convidado que depois vira real": é A conta desde o primeiro segundo, só sem dado pessoal. O servidor (`server/contas.js`) a cria, emite um **token opaco** (`crypto.randomBytes(32)`, base64url) que fica no aparelho (`localStorage['incursion:token']`), e o **perfil nasce no servidor** (via `src/perfil.js` importado — mesma fonte do cliente, grant de `data/economia.json`). O token reabre a MESMA conta ao fechar/reabrir. Medido na captura: primeira abertura mostra o **portão de idade, não um login**; nenhum campo de senha/e-mail existe.

**2. FAIXA DE IDADE, não data (Lei 15.211).** Menor de 18 não pode ter aleatoriedade paga. Perguntamos a **faixa** (`'menor'` | `'maior'`) e guardamos só isso — **nunca a data de nascimento** (menos dado pessoal = menos risco LGPD). A faixa é **obrigatória na criação** (o servidor recusa `criarConta` sem faixa válida): o dever legal é ter perguntado e registrado. Se o jogador mentir, o dever já foi cumprido.

**3. EXCLUSÃO DE VERDADE — o que apaga, o que preserva.** `excluir(token)` **remove a linha inteira** do cadastro e regrava o arquivo — não é marcação de "apagado". Depois disto o token não corresponde a nada e o aparelho volta à primeira abertura. **O QUE APAGA:** tudo — id, token, faixa, ranque, nick, perfil (coleção/campanha/maestria), data de criação. **O QUE PRESERVA:** **nada.** Não há dado pessoal a reter (conta anônima, sem e-mail/nome/data) nem obrigação legal de guardar; preservar qualquer coisa contradiria "exclusão de verdade". Se um dia o ranqueado exigir antifraude, isso será uma decisão separada com sua própria justificativa — não um resíduo silencioso agora. Medido: `contas 1 → 0`, token apagado, portão de volta.

**4. O NICK vem DEPOIS (PvP/F5.3), o campo já existe.** A conta nasce com `nick: null`. Reservar o campo agora evita refazer o cadastro depois. **Plano registrado** (`server/contas.js → planoDoNick()`): **unicidade** por índice único sobre o nick normalizado (minúsculas, sem acento), com **reserva atômica no servidor** (quem grava primeiro leva; o segundo recebe `nick_em_uso`) — nada de unicidade no cliente (dá corrida); **palavra ofensiva** por lista de bloqueio + checagem de leetspeak (0→o, 3→e) no servidor, curadoria versionada em `data/` (some do cliente, atualiza sem publicar app novo).

**5. MIGRAÇÃO ÚNICA do perfil de DEV, ranque zero.** O perfil de desenvolvimento do dono (localStorage, com campanha/coleção/maestria) migra UMA vez para a conta anônima. Como se separa do jogador novo: o cliente captura, **antes de `iniciar()` persistir**, se já havia perfil no disco (`perfilPreexistente`) — instalação nova nasce com o disco vazio, nasce no servidor e **nunca migra**. A migração é marcada (`incursion:conta-migrou`) para acontecer só uma vez. O **ranque começa em zero de qualquer forma** (§221-d): o servidor **ignora** qualquer ranque que venha no perfil migrado. Medido: perfil migra (gema/campanha/maestria preservadas), ranque = 0.

**6. O PROTOCOLO GANHA AUTENTICAÇÃO (segue v1).** Depois do handshake, **toda mensagem de jogo leva o token**; sem token válido = `recusado` com motivo claro, nunca silenciosa. Tipos que NÃO exigem token: `ola` (handshake), `criarConta`, `entrar` (ainda não há token, ou o token É o corpo). Isto **quebrou legitimamente o `tests/servidor.test.js` da F5.0** — que mandava `montar`/`agir` sem token — e o teste foi **corrigido para autenticar primeiro** (cria conta anônima, usa o token): o motor autoritativo continua batendo o hash da referência em cada passo, agora sob autenticação.

**ONDE A CONTA MORA E POR QUE É DORMENTE SEM SERVIDOR:** `src/conta.js` é a borda do cliente — lógica pura (sem DOM, `module.exports` para o Node testar), com um **transporte injetável** (WebSocket no app; um duble nos testes, já que o jsdom não tem WebSocket). O app só cria o transporte quando é servido por `http(s)://` (por `file://` retorna `null`); sem servidor, **nada de conta aparece e o app roda 100% local** — é o que mantém as 28 suítes anteriores e o modo offline intactos. As sobreposições (portão de idade, painel de conta) são **DOM próprio fora do `#stage`** — não passam pelo `render()` nem tocam nas telas.

**PROVA:** `tests/contas.test.js` — (1) o cadastro (cria/entra/exclui real, faixa, ranque zero mesmo migrado, token não vaza nas projeções); (2) o servidor por WebSocket (mensagem de jogo sem token RECUSADA, `criarConta`→token, token ruim recusado, token bom aceito, exclusão real invalida o token); (3) o módulo cliente com transporte injetável (1ª abertura pede faixa, cria em silêncio, guarda o token, reabre sem portão, migra 1×, exclui→estado inicial, dormente sem servidor). Suítes verdes (30 no `npm test`). Capturas contra o `dist` servido pelo servidor de verdade.

**ESCOPO:** nada de pareamento, ranque de verdade, nick ativo ou vínculo Google/Apple aqui. Só a conta, a faixa, a exclusão, o token e a migração.
...
**hercules RESISTE — o rider fiel é construído mas NÃO fecha. Raiz: o stacking é ATIVO-OPCIONAL que o solver PULA.** Rider correto: `buffNoAbate{buff:'dmgUp', v:20, quem:'hercules'}` (§187 estendeu buffNoAbate com `v`=valor-mínimo e `quem`=nomeado; dmgUp FUNDE, l.502, então v20 = 5 usos de Os Doze Trabalhos) + `proibirSlotProprio{milagre}`. Dois becos:
- **com curador que dá dmgUp (oxum): ACIDENTE** — oxum·Águas de Oxum→hercules alimenta o dmgUp (o caminho tinha 10 casts de oxum, só 2 de hercules). O v20 vem do ALIADO, não do grind. Não-protagonista (o "susanoo carrega o strip" de novo).
- **com curador SEM dmgUp (change): o solver IGNORA o grind** — vence via perseu/change e hercules fica bystander (H≈v×3, ~0 self-stacks, mesmo em v8). O gradiente ×3 não força um ativo que a vitória não exige. E o "sem milagre" tira o burst → set tanky mata o time, set squishy morre antes de 5 casts. Banda vazia.
**hercules é saída-por-FERRAMENTA-FRACA (§177):** o rider é bom, o KIT (stacker +4/cd2, fraco-sem-milagre) não o sustenta. Fase 4 (rework de kit), não Fase 2.

**shutendoji RESISTE — rider fiel construído mas NÃO NAVEGÁVEL. Raiz: o recurso é LENTO/REATIVO.** Rider correto: `acumulo{orbesRoubados, 8}` (a fonte que saci usa; a prosa pede "roube 8 com o Torpor"). Mas o roubo do shutendoji é REATIVO (Saké Envenenado: "quando o inimigo AGIR, rouba 1 orbe") + Banquete (milagre lento). O solver mata os inimigos ANTES de eles agirem-torporados → 0 roubos (H=limiar em todo N). Inimigos tanky (p/ sobreviver e serem roubados) QUEBRAM a vitória (H=248). **O gradiente ×1 do acumulo serve p/ recurso RÁPIDO (saci rouba direto) e NÃO p/ um lento-reativo.** É o §176 num eixo novo: predicado navegável depende da VELOCIDADE do recurso, não só do gradiente existir.

**META-PADRÃO (o que separa yamato dos outros dois) — dívida-de-conteúdo ≠ dívida-de-backlog nem sempre se resolve com o predicado certo.** O rider fiel só FECHA se a ferramenta for **protagonista-única E rápida/decisiva** (yamato). Se for **opcional** (hercules: a vitória não precisa do grind) ou **lenta/reativa** (shutendoji: o roubo não acompanha a corrida-p/-vencer), o solver a PULA e o gradiente não a força. Isso GENERALIZA o "ativo-opcional invertido" (§179, AUTO-MORTE) p/ além da auto-morte: **qualquer rider cuja ferramenta a vitória não exige é anti-solver.** yamato escapou porque o mass-strip é único (protagonista de graça) e o pré-buff o fez decisivo-numa-cast.

**DECISÃO:** yamato re-carimbado (dívida paga). hercules e shutendoji FICAM com o carimbo antigo (VENCÍVEL, lição-errada) — **não se pode publicar Provação invencível**, e o rider certo não fecha no kit atual. Reclassificados de "dívida-real-consertável-na-Fase-2" (§185) p/ **saída-por-ferramenta-fraca → rework de kit na Fase 4**. As 3 ferramentas construídas (stripBuffsInimigo, buffNoAbate{v,quem}, statusTurnos §186) ficam corretas e reusáveis. **Pendente p/ o dono:** aceitar a lição-errada publicada até a F4, OU rebalancear os kits (hercules: stacker mais rápido / milagre que não anula; shutendoji: roubo não-reativo), OU um mecanismo mais forte (abate-por-slot-com-N-stacks + gradiente forte).

## §186 — A CAUDA FECHADA: orfeu carimbado (statusTurnos, uptime-de-status relaxado), tsukuyomi SAI (assimetria Dia×Noite estrutural); e o PADRÃO da dívida real (golpe-final virou acumulação).

**`statusTurnos{status, limiar}` (motor §186) — o uptime-de-STATUS relaxado.** Espelha o `estadoTurnos` (§178): o turno-event ganha `statusInimigo` (tipos de debuff em inimigos vivos, UMA linha no log — como o `campo` do §178), e o predicado conta TURNOS distintos com ≥1 inimigo carregando o status. Testado §24, gradiente ×30. Relaxa o uptime-de-status ESTRITO (irrealizável, §178).

**orfeu "Canção do Submundo" (Provação d2) CARIMBADO — `statusTurnos{adormecido, 2}`.** orfeu·perseu·zeus vs fenrir·kali·babi dl12 (+bootstrap Umbra:6) = VENCÍVEL nós 43. **Babá LIMPO:** trocar orfeu por medusa (Umbra, sem sono) → statusTurnos trava em H=60 (0 turnos-de-sono; a Canção Órfica é a única fonte). **A banda é ≥2 e SÓ ≥2:** ≥3 trava (H=30), ≥4 (H=60), ≥5 (H=90) — a Canção Órfica dorme por 2 turnos e ACORDA se o alvo sofrer dano; manter alguém dormindo E vencer só cobre ~2 turnos. É o mesmo teto-de-uptime do §178 (amaterasu Dia≥4), aqui em ≥2. nós 43 = raso, mas load-bearing.

**tsukuyomi SAI — saída-por-FERRAMENTA-FRACA (a assimetria Dia×Noite que eu suspeitava é ESTRUTURAL).** A mesma forma (campo-uptime, `estadoTurnos`) FECHA para amaterasu (Dia≥4) e NÃO fecha para tsukuyomi (Noite): 0 turnos-de-Noite contra o set do catálogo (amaterasu·ra·lugh, que a desfazem) E 0 mesmo contra set amigável (shiva·durga·fenrir). **A causa é do KIT, medida:** amaterasu tem DOIS setters de Dia (Amanhecer + Luz da Caverna); tsukuyomi tem UM de Noite (Anoitecer, custo Umbra:2) → a economia não paga a cobertura contínua (a parede do dagda, §174). O passivo da amaterasu (Soberana Celestial) só LÊ o Dia (redução), não o mantém — então nem é isso; é a contagem de setters + economia. **Volta com um deus de Noite com kit mais forte de campo** (2 setters ou passiva-mantenedora). O `estadoTurnos` fica provado nos dois sentidos: o predicado está certo; o kit do tsukuyomi é que não sustenta o campo dele.

**A CAUDA (baratas de uptime/HP) ESTÁ FECHADA.** Resolvidas: amaterasu, aquiles, ares, mula, afrodite, atena, orfeu (carimbadas); curupira, tsukuyomi (saídas). Não sobra barata.

**PADRÃO DA DÍVIDA REAL (dono, §185 grupo 4) — REGISTRAR: "golpe-final virou acumulação".** As 3 dívidas reais (hercules, shutendoji, yamatotakeru) têm a MESMA forma de erro: **quando o rider da prosa é "faça X NO GOLPE FINAL", eu troquei por "ACUMULE X"** — que é mais fácil de medir e ENSINA OUTRA COISA. **Golpe-final é TIMING (o estado no instante do abate); acumulação é VOLUME (a soma ao longo da luta).** São eixos diferentes. **Onde derivei mal:** o modo `final` com leitura-de-instante-do-abate era mais chato de escrever que o `acumulo` de-log, e eu peguei o atalho. **O §46 do golpe-final JÁ EXISTE (`buffNoAbate`, `abatePorSlot`)** → os 3 são re-escrevíveis SEM motor novo. Regra: rider de golpe-final NUNCA vira acumulação; se a prosa diz "no golpe final", o predicado é modo-final-no-abate, não `acumulo`.
...
- **`soloSobrevivente{quem}`** (modo:'final') — VENCER com só o título vivo (todos os OUTROS aliados caídos). INVERSO do `semPerderAliado`. Gradiente ×1000 por outro-aliado-vivo (anti-greedy: o solver preserva aliados por padrão). Título morto = beco (100000).
- **`tituloCaido{quem}`** (modo:'final') — o título tem de estar MORTO no golpe final (AUTO-MORTE real). Espelho NEGATIVO do `reviveAliado`: sem gradiente que premie a morte, o solver protege o título e o rider é insatisfazível (a tensão do dono). ×1000 recompensa deixá-lo cair. Combina com hpNoFim (ymir) e com o dead-passive do motor (mesmoMorto §123).

**mimir "A Cabeça Falante" CARIMBADA — Provação d2 (requisito: odin). O dead-passive É load-bearing, e o SET DO CATÁLOGO é que o prova.** mimir·odin·zeus vs **thor·tyr·heimdall (os três Guardiões)** dl14, `tituloCaido{quem:mimir}` = VENCÍVEL nós 36. Babá elemento-matched (corpo squishy Umbra que TAMBÉM morre, mas SEM o `+6 mesmoMorto`): change H=86, cuca H=129 (travam). **A TANKINESS dos Guardiões é o que torna o +6/turno-morto DECISIVO** — num set mole (thor·tyr·saci) o babá quase não mordia (H=1/H=4); nos três Guardiões morde fundo (H=86/129), porque só o +6 acumulado quebra as muralhas. **nós 36 = busca RASA, mas protagonismo REAL** (o nº de nós mede dificuldade-de-busca, não validade-de-design; o babá mede protagonismo, e passa limpo). Dá ao **odin o papel-sem-Provação-própria** — a "saída digna para um deus mismatched" que o dono pediu (§183: odin saiu por rider-mismatched; aqui volta como aliado-requisito).

**erinias SAI — saída-por-FERRAMENTA-FRACA (2ª espécie, §177). O predicado CONFIRMA o solver-alinhamento, mas a kit não FINALIZA.** `soloSobrevivente` funciona como previsto: o gradiente ×1000 + a passiva dela (Fúria Vingativa, +8 por aliado morto) puxam no MESMO sentido → o estado-solo É alcançado (H<1000 em quase todo config). **MAS erinias-sozinha não fecha o 1vN:** platô em H=6–88 (dica) em ~13 configs (deadlines 13–16, bootstrap Umbra 6–10, sets de tanky a três-squishies saci·cuca·exu). Uma controladora squishy não sobrevive/não finaliza sozinha contra o resto. **Achado lateral: o pré-caído é ESSENCIAL** — SEM ele, H=1000–2000 (o solver mantém aliados vivos e vence normal, ignorando o solo). O RIDER é bom e a DIREÇÃO é certa; a FERRAMENTA (kit da erinias) é que não entrega. **Volta com um carry-solo mais forte** (bruiser com lifesteal/burst). Predicado pronto, à espera do deus — como o rider-mismatched, mas por motivo distinto (aqui a forma fecharia para deus mais FORTE, não OUTRO).

**ymir SAI — saída-por-rider-MISMATCHED (3ª espécie, a do odin). Confirmado com expectativa baixa (dono).** `tituloCaido{ymir}` + 2×`hpNoFim<=30` vs sets de fogo/agressão: platô H=56–218 (dica). ymir é **GUARDIÃO (tank)** — o rider "cai primeiro p/ a cura-de-morte sustentar o time" quer um corpo SACRIFICÁVEL, e o tank é o ÚLTIMO a cair (briga com o arquétipo). Mesma assinatura do odin (§181). **Volta quando houver rider de TANQUE** (ou um deus squishy de auto-sacrifício-por-cura). Não forçar.

**FECHAMENTO DO LOTE AUTO-MORTE — "não é família" (§182) CONFIRMADO pela medição.** Os 4 se resolvem por 4 caminhos distintos: bennu FECHA por self-revive (REVIVE-família, §183), mimir FECHA por dead-passive (tituloCaido), erinias SAI por ferramenta-fraca, ymir SAI por rider-mismatched. **Os dois predicados invertidos nascem e JÁ se provam** (mimir usa tituloCaido; erinias diagnostica com soloSobrevivente) — §176 respeitado (nasceram com gradiente). **As 3 espécies de saída agora têm ≥2 instâncias cada:** rider-impossível {nuwa}; ferramenta-fraca {oni, boitatá, curupira, erinias}; rider-mismatched {odin, ymir}.
