...
## §277-ajustes — TRÊS acertos do §277 (dois de texto aplicados; um de medida, medido e PROPOSTO, aguardando o dono).

**1. NOMES DOS DEUSES — medido, NÃO aplicado (o dono pediu para ver medido antes de encurtar).**
- **1a (crescer a caixa) NÃO é viável.** No piso 780 o cartão tem 138px; a fileira dos três deuses já ocupa **142px na caixa atual de 44px** (encosta na borda). O teto físico por nome é **~46px** (138÷3). Caber "Sun Wukong" (60px) exigiria caixa 60 → fileira 190px → **52px para fora do cartão**. Crescer a caixa arromba o cartão; descartado.
- **1b (encurtar no DADO, campo próprio — precedente `curto` do §262).** Medi os quinze no rótulo real (8px Cinzel 600, rede bloqueada). Cabem (≤44): Zeus 22 · Atena 29 · Odin 24 · Thor 25 · Loki 22 · Rá 12 · Ísis 16 · Osíris 29 · Susanoo 42 · Nezha 30 · Nüwa 26. **CORTAM:** Poseidon **46** · Amaterasu **52** · Tsukuyomi **52** · Sun Wukong **60**. **Proposta medida (aguardando o dono):** só **Sun Wukong → "Wukong" (40px)** tem forma curta limpa e resolve o caso real. **Poseidon (46)** fica a 0-2px do teto — some sem reticência perceptível; sugiro deixar. **Amaterasu/Tsukuyomi (52)** são nomes próprios de uma palavra SEM forma curta graciosa ("Amateras" ainda dá 46 e é feio; "Tsukuyo." 43 mas com ponto) — a 780/951 cortam ~6px ("Amateras…"), cabem só a 1200. **Não apliquei nada** (o portão do dono é "propor medido antes de aplicar"): a escolha entre (a) aceitar a reticência de ~6px de Amaterasu/Tsukuyomi, (b) uma abreviação que o dono aprove, é do dono. Fonte NÃO reduzida (pioraria 15 rótulos para salvar 4).
- **§263 confirmado:** um campo `curto` para a trinca NÃO entra no carimbo de combate — `curto` está na deny-list `TELA_TOPO` da projeção de combate (`src/provacao.js`), e o carimbo hasheia o catálogo de deuses/bestiário, nunca as escadas de `data/dominios`. Nenhuma Provação re-carimba.
...
**3. PALETAS — aprovadas as duas (Nórdica azul-gelo, Japonesa).** Os valores da Nórdica derivada ficam REGISTRADOS no `src/shell.html`, na regra `.dcard--nórdica`, junto das outras quatro, com comentário de "aprovada — não re-derivar" (a próxima sessão não deriva de novo): `--tag:#BFE3F2 · --frame:rgba(150,205,230,.7) · --glow:rgba(90,165,205,.2) · --bodyA:rgba(10,20,32,.92) · --bodyB:rgba(6,12,20,.96) · --artFade:rgba(8,18,30,.9) · --veilTop:rgba(8,18,30,0) · --veilBottom:rgba(6,13,22,.94) · --embA:#1C3A4E · --embB:#08161E`. Monogramas OL·AS·DU·TK·CE ficam (placeholder; somem com a arte).

**Arquivos:** `src/ui/home.js` (linha-guia), `src/shell.html` (comentário da Nórdica), `DECISOES.md`, `ESTADO.md`. (O `curto` dos deuses NÃO foi tocado — aguarda o dono.)
